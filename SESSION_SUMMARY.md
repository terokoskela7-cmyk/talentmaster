...
- **MAS-syväanalyysi KERROS 4 — live-trendi (async-haku).** Nyt (commit `0e161b4`) KERROS 4 lukee profiilin `mas_historia`-kentästä `[{ms, kmh?, pvm}, …]` (synkroninen render → ei alikokoelmakyselyä, §26). **Async-parannus:** hae pelaajan aiemmat MAS-arvot `testitulokset/`-alikokoelmasta (placeholder → fetch → DOM-patch), laske muutos m/s:ssä, näytä suunta. **Aktivoituu automaattisesti** kun Leon kaltaisille pelaajille kertyy ≥2 mittauspistettä. Koskee VP_v25 `_masSyva` + Pelaaja_v7 `rMinaMAS` (showcase). Rinnalla: kirjoita `mas_historia` profiiliin tuonnissa (recalc/Excel) jotta synkroninen versio toimii ennen async-refaktorointia.
