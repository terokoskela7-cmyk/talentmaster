...
- **MONIPÖYTÄTUKI — KAKSIPASSINEN PARSINTA** (`kaksipassi-v3`, 2026-05-26; **korvasi** aiemman jono/sija-reset/sarakemäärä/otsikkoraja-heuristiikan, joka oli hauras taulukkorajojen suhteen): sama PDF voi sisältää useita ikäluokkia (P12+P10+P9). **Passi 1** etsii otsikkorivit (`IKAOTSIKKO = /^(T|P)(\d+)$/i`, koko rivin teksti = täsmälleen esim. "P12") ja tallentaa niiden rivivälit `{ikaluokka, sukupuoli, ika, alku, loppu}` — osion `loppu` = seuraavan otsikon indeksi. **Dedup:** sama otsikko voi toistua sivunvaihdon yli (P9 esiintyy sivuilla 1 JA 2) → saman ikäluokan osiot yhdistetään (`loppu` laajenee viimeiseen esiintymään); duplikaattiotsikkorivit ohitetaan Passi 2:ssa (parsinta palauttaa null otsikkoriville). **Passi 2** parsii kunkin osion rivit erikseen omalla **`ctx`-OBJEKTILLA** `{ikaluokka, ika, sukupuoli}` — **ctx PAKKO olla objekti, ei string** (pelkkä string → `ctx.ika` undefined → P12-sarakekartoitus 7-sarakkeiseksi + TKI/merkki laskematta). **P12-minimi `nums.length >= 5`** (osa P12-riveistä < 6 numeroa), muut ikäluokat >= 6. `OSIO:`-konsoliloki jätetty diagnostiikkaan. **Testattu Sibbon tulosteella: P12 23 · P10 26 · P9 15 = 64 riviä, 62 yhdistyi nimellä, 0 duplikaattia** ✅. (Vanha oletus "kaikki otsikot ensin, sitten data" EI pitänyt — rakenne oli otsikko→data lomittain + P9 toistuu sivunvaihdossa.)
...
- **Merkki AINA kokonaistuloksesta** (`tkLaskeMerkki(kokonaistulos,…)`), EI TKI:stä. Excel/PDF-tuonti + recalc johtavat merkin kokonaistuloksesta. Renderöinti `_tkiMerkkiM`/`_tkiMerkkiVP` lukee **VAIN `tki_merkki`-kentästä** (ei TKI-johdettua fallbackia, `const m = merkkiKentta || null`). Recalc kirjoittaa `tki_merkki: null` myös puuttuessa → ylikirjoittaa vanhan väärän merkin.
- **Admin-työkalut (SA only):** **↻ Laske TKI uudelleen** -nappi Excel-tuonnin topbarissa — laskee `tki_viimeisin` + pikakentät uudelleen kunkin pelaajan viimeisimmästä tekniikkakilpailu-tuloksesta. **Ikä johdetaan pelaajan `syntymaVuosi`-kentästä** (EI testituloksen ikäluokasta — ikäluokka kertoo kilpailusarjan, ei pelaajan todellista ikää), **kilpailuvuosi luetaan testituloksen `d.pvm`-kentästä** (ika = kilpailuvuosi − syntymaVuosi). Ei vaadi PDF:ää. **Kun `tki == null` recalc NOLLAA** `tki_viimeisin` + `tki_merkki` pelaajadokumentista (ei enää ohita pelaajaa) → vanha väärä arvo poistuu myös ilman uutta tulosta. **`siivoaBugisetTulokset(seuraId, ikaluokka, maxKokonais, dryRun=true)`** konsolifunktio — poistaa testitulokset joissa `kokonaistulos_s < maxKokonais` (dry-run oletus listaa, `false` poistaa).
