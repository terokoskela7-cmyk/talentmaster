...
- A5 🟡 KÄYNNISSÄ: `luotu` kaksi vikaluokkaa — tyyppi-mismatch (Master ISO vs Timestamp-kyselyt) + **puuttuva kenttä** (Pelaaja kirjoittaa `paivitetty` muttei `luotu`:a → Vanhempi `orderBy('luotu')` sulkee kirjaukset pois). Plan B: (1) ✅ Rules-vartija `luotuLuontiKelpaa`/`luotuPaivitysKelpaa` (havainnot/kirjaukset/kehut) + 6 rules-testiä, kääntyy, EI deployattu. (2) ✅ writer-fix: Master 3498/3537→serverTimestamp, Pelaaja kirjaus+takautuva→`Timestamp.fromDate(new Date(pvm))` (TUOTEPÄÄTÖS luotu=treenipäivä). Cascade `_luotuToMs()` (3861/3879/5508) + `adar_pvm` pidetään ISO-stringinä (VP-turva). (3)⬜ migraatio (collectionGroup, havainnot konvertoi, kirjaukset backfill=Timestamp.fromDate(pvm)). (4)⬜ deploy vartija VIIMEISENÄ. ⚠ Vartijan deploy ENNEN migraatiota rikkoo live-kirjoitukset!
