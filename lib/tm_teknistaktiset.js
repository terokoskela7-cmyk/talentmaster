...
function tmTtVaihe(p) {
  ...
  if (ika <= 9) return 'perus';        // §0a: ≤9 perus · 10–13 yhteispeli · 14 silta/pelipaikka · ≥15 pelipaikka
  if (ika <= 13) return 'yhteispeli';
  // I3a §C.3 — pelipaikkaportti joustavaksi: aikainen erikoistuja (pelipaikka asetettu) avaa pelipaikkavaiheen jo U14;
  // ilman pelipaikkaa U14 = silta. U15+ = pelipaikka viimeistään (standardi). Ei kovaa katkaisua.
  if (ika <= 14) return (p.tt_positio_aktiivinen || p.positio) ? 'pelipaikka' : 'silta';
  return 'pelipaikka';
}
...
// I3a §E — harjoitesuodatus tägeillä/jalalla. Harjoitteille valinnaiset additiiviset kentät:
//   `tagit:['placement'|'power'|'first-time'|'variety'…]` + `jalka:['dominoiva'|'heikompi'|'molemmat']`.
// opts.tagit/opts.jalka → priorisoi osuvat harjoitteet; tägitön sisältö säilyy fallbackina (EI rikkoudu ilman tägejä).
// Resolver (tm_kehityspolku) antaa Y-H9 + harjoite_tagit → harjoitevalinta kutsuu tätä. Sisällön tägitys = erillinen sisältötyö.
function tmTtHarjoitteetSuodata(avainTaiKoodi, opts) {
  opts = opts || {};
  var kaikki = tmTtHarjoitteet(avainTaiKoodi);
  if (!kaikki.length) return kaikki;
  var tagit = (opts.tagit && opts.tagit.length) ? opts.tagit : null;
  var jalka = opts.jalka || null;
  if (!tagit && !jalka) return kaikki;
  var osuu = kaikki.filter(function (h) {
    var okTag = !tagit || (Array.isArray(h.tagit) && h.tagit.some(function (t) { return tagit.indexOf(t) >= 0; }));
    var okJalka = !jalka || jalka === 'molemmat' || (Array.isArray(h.jalka) && (h.jalka.indexOf(jalka) >= 0 || h.jalka.indexOf('molemmat') >= 0));
    return okTag && okJalka;
  });
  return osuu.length ? osuu : kaikki;   // ei tägitettyä osumaa → koko lista (graceful §E)
}
...
var TM_TT_API = {
  ...
  tmTtKysymykset: tmTtKysymykset, tmTtHarjoitteet: tmTtHarjoitteet, tmTtHarjoitteetSuodata: tmTtHarjoitteetSuodata, tmTtPelaaja: tmTtPelaaja
};
