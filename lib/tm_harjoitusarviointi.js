...
(function () {
  ...
  function _kriteeriRivi(k) {
    ...
    if (k.tyyppi === 'toggle') {
      ...
      kontrolli = '<div style="display:flex;gap:8px">'
        + '<button type="button" onclick="_haSetA8(true)" style="flex:1;padding:8px;border-radius:7px;border:.5px solid ' + (on ? 'var(--ha-accent)' : 'var(--border)') + ';background:' + (on ? 'var(--ha-accent-bg)' : 'var(--surface)') + ';color:' + (on ? 'var(--ha-accent)' : 'var(--ink2)') + ';cursor:pointer;font-size:13px">Kyllä</button>'
      ...
      kontrolli = '<div style="display:flex;align-items:center;gap:10px">'
        ...
        + ' oninput="document.getElementById(\'haval_' + k.id + '\').textContent=this.value+\'' + (k.tyyppi === 'pct' ? ' %' : '') + '\'" style="flex:1;accent-color:var(--ha-accent)">'
        + '<span id="haval_' + k.id + '" style="min-width:42px;text-align:right;font-family:\'Cormorant Garamond\',serif;font-size:20px;color:var(--ha-accent)">' + _esc(disp) + '</span></div>'
  ...
  function _kriteeriRiviB(k) {
    ...
    for (var i = 1; i <= 5; i++) {
      var sel = (cur === i);
      napit += '<button type="button" onclick="_haSetB(\'' + k.id + '\',' + i + ')" style="flex:1;padding:9px 0;border-radius:7px;border:.5px solid ' + (sel ? 'var(--ha-accent)' : 'var(--border)') + ';background:' + (sel ? 'var(--ha-accent-bg)' : 'var(--surface)') + ';color:' + (sel ? 'var(--ha-accent)' : 'var(--ink2)') + ';cursor:pointer;font-size:14px;font-weight:600">' + i + '</button>';
    }
    napit += '</div>';
  ...
  // ── White-label (§5): brändi-aksentti override. Carbon säilyy pohjana; vain aksentti+logo muokattavissa.
  // Saavutettavuus: teksti aksentin päällä ≥4.5:1 (WCAG AA) JA aksentti luettava tummalla UI-bg:llä ≥3:1
  // → muuten teal-fallback (#28B090). Oletus (ei brandia) = teal → näkymä identtinen entiseen.
  function _hexN(h) { h = String(h == null ? '' : h).trim().replace('#', ''); if (h.length === 3) h = h[0] + h[0] + h[1] + h[1] + h[2] + h[2]; var n = parseInt(h, 16); return (h.length === 6 && !isNaN(n)) ? { r: (n >> 16) & 255, g: (n >> 8) & 255, b: n & 255 } : null; }
  ...
  function _kontrasti(h1, h2) { var a = _hexN(h1), b = _hexN(h2); if (!a || !b) return 0; var l1 = _lum(a), l2 = _lum(b); return (Math.max(l1, l2) + 0.05) / (Math.min(l1, l2) + 0.05); }
  function _brandiVars(brandi) {
    var DEF = { acc: 'var(--teal)', ink: '#fff', bg: 'rgba(40,176,144,.15)', bg2: 'rgba(40,176,144,.12)' };
    if (!brandi || !brandi.accent_hex) return DEF;
    var rgb = _hexN(brandi.accent_hex); if (!rgb) return DEF;
    var ink = brandi.accent_paalle_teksti_hex || '#fff';
    if (_kontrasti(brandi.accent_hex, ink) < 4.5 || _kontrasti(brandi.accent_hex, '#161614') < 3.0) return DEF;   // riittämätön → teal-fallback
    return { acc: brandi.accent_hex, ink: ink, bg: 'rgba(' + rgb.r + ',' + rgb.g + ',' + rgb.b + ',.15)', bg2: 'rgba(' + rgb.r + ',' + rgb.g + ',' + rgb.b + ',.12)' };
  }
  // CSS-muuttuja-string modaalin juureen (kaikki var(--ha-accent*) periytyy haInner:iin).
  function _brandiCssVars(brandi) { var b = _brandiVars(brandi); return '--ha-accent:' + b.acc + ';--ha-accent-ink:' + b.ink + ';--ha-accent-bg:' + b.bg + ';--ha-accent-bg2:' + b.bg2; }
  ...
  function _render() {
    ...
    var h = '<div style="display:flex;justify-content:space-between;align-items:flex-start;gap:10px;margin-bottom:12px">'
      + '<div>' + ((_O.brandi && _O.brandi.logo_url) ? '<img src="' + _esc(_O.brandi.logo_url) + '" alt="" style="height:26px;max-width:130px;object-fit:contain;display:block;margin-bottom:6px">' : '')
      + '<div style="font-family:\'Cormorant Garamond\',serif;font-size:24px;color:var(--ink);line-height:1.1">' + _esc(_O.otsikko || 'Arvioi harjoitus') + '</div>'
      + '<div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:6px">' + _chip(malliLabel, 'var(--ha-accent)') + _chip(_S.ikavaihe === 'lapsuus' ? 'Lapsuus 5–11' : 'Nuoruus 12–15', 'var(--blue)') + (isA ? '' : _chip(_S.arviointitapa === 'itsearvio' ? 'Itsearvio' : 'Havainnointi', 'var(--amber)')) + '</div></div>'
    ...
    if (mallit.length > 1 && !_O.malliLukko) {
      ...
      mallit.forEach(function (m) {
        ...
        h += '<button type="button" onclick="_haSetMalli(\'' + m + '\')" style="flex:1;padding:9px;border-radius:8px;border:.5px solid ' + (sel ? 'var(--ha-accent)' : 'var(--border)') + ';background:' + (sel ? 'var(--ha-accent-bg2)' : 'var(--surface)') + ';color:' + (sel ? 'var(--ha-accent)' : 'var(--ink2)') + ';cursor:pointer;font-size:12px;font-weight:600">' + lbl + '</button>';
      });
      h += '</div>';
    }
    ...
    h += '<div>Ikävaihe: <button type="button" onclick="_haSetIka(\'lapsuus\')" style="padding:4px 10px;border-radius:6px;border:.5px solid ' + (_S.ikavaihe === 'lapsuus' ? 'var(--ha-accent)' : 'var(--border)') + ';background:' + (_S.ikavaihe === 'lapsuus' ? 'var(--ha-accent-bg2)' : 'transparent') + ';color:' + (_S.ikavaihe === 'lapsuus' ? 'var(--ha-accent)' : 'var(--ink2)') + ';cursor:pointer;font-size:11px">Lapsuus</button> '
      + '<button type="button" onclick="_haSetIka(\'nuoruus\')" style="padding:4px 10px;border-radius:6px;border:.5px solid ' + (_S.ikavaihe === 'nuoruus' ? 'var(--ha-accent)' : 'var(--border)') + ';background:' + (_S.ikavaihe === 'nuoruus' ? 'var(--ha-accent-bg2)' : 'transparent') + ';color:' + (_S.ikavaihe === 'nuoruus' ? 'var(--ha-accent)' : 'var(--ink2)') + ';cursor:pointer;font-size:11px">Nuoruus</button></div>';
    if (!isA && !_O.tapaLukko) {   // tapaLukko (esim. valmentaja = kiinteä itsearvio) → ei toggleria
    ...
    if (isA) {
      ...
      /* ITSEREFLEKTIO VAIN ITSEARVIOSSA (D1:n jäänne).
         Lohko renderöityi aiemmin kaikille malli B:n lomakkeille, joten kun VP
         tekee HAVAINNOINTIA, lomake pyysi VP:tä täyttämään valmentajan oman
         itsereflektion ("Mikä onnistui? / Mitä tekisin toisin? / Oma
         kehityskohde valmentajana"). Itsereflektio on valmentajan oma arvio
         itsestään — ei havainnoijan tehtävä. Masterin valmentaja-itsearvio
         (tapaLukko + arviointitapa:'itsearvio') näyttää sen ennallaan. */
      if (_S.arviointitapa === 'itsearvio') {
      var r = _S.reflektio;
      h += '<div style="background:var(--card);border:.5px solid var(--ha-accent);border-radius:10px;padding:12px 14px;margin-bottom:10px">'
        + '<div style="font-size:12px;font-weight:600;color:var(--ha-accent);text-transform:uppercase;letter-spacing:.04em;margin-bottom:8px">Itsereflektio</div>'
    ...
    h += '<button type="button" onclick="_haTallenna()" id="haTallennaBtn" style="width:100%;margin-top:6px;padding:11px;border:none;border-radius:9px;background:var(--ha-accent);color:var(--ha-accent-ink);font-size:14px;font-weight:600;cursor:pointer">Tallenna arviointi</button>';
    inner.innerHTML = h;
  }
  ...
  window._haSetA8 = function (b) { _keraa(); _S.a8 = b; _render(); };   // bug-1: säilytä a1–a7-syöte togglatessa
  ...
  window._haTallenna = async function () {
    ...
    try {
      ...
      // Karsi doc vain aktiivisen mallin kenttiin (mallin vaihto kerryttää _S:ään ristiin a*/b*)
      var avaimet = _S.malli === 'palloliitto' ? ['a1', 'a2', 'a3', 'a4', 'a5', 'a6', 'a7'] : ['b1', 'b2', 'b3', 'b4', 'b5', 'b6', 'b7'];
      var vast = {}, tasm = {};
      avaimet.forEach(function (k) {
        if (_S.vastaukset[k] != null) vast[k] = _S.vastaukset[k];
        if (_S.tasmennykset[k] != null && _S.tasmennykset[k] !== '') tasm[k] = _S.tasmennykset[k];
      });
      var doc = {
        malli: _S.malli,
      ...
      /* Reflektio tallentuu VAIN itsearviosta — havainnointidokkiin ei kirjata
         tyhjää objektia, joka näyttäisi siltä että havainnoija jätti
         itsereflektion täyttämättä.
         HUOM rakenne: ehtoa EI saa litistää muotoon
           `if (malli === 'valmennustaidot' && tapa === 'itsearvio') … else …`
         koska silloin malli B + havainnointi putoaisi malli A:n else-haaraan ja
         saisi `henk_palaute`-kentän joka ei kuulu sille lainkaan. */
      if (_S.malli === 'valmennustaidot') {
        if (_S.arviointitapa === 'itsearvio') doc.reflektio = _S.reflektio;
      } else {
        doc.henk_palaute = (_S.a8 === true);
      }
      ...
      try {
        ...
        if (typeof laskeValmentajaHarjoitusKooste === 'function') {
          ...
          // update() EI luo puuttuvaa dokumenttia (set(merge) loisi) — pikakenttä on liite
          // olemassa olevaan profiiliin, ei profiilin luoja. Jo catch (e2):ssa → hiljainen ohitus:
          // arviointi itse on jo tallennettu harjoitusarvioinnit-kokoelmaan, eikä puuttuva
          // profiili saa kaataa sitä.
          await db.collection('seurat').doc(sid).collection('kayttajat').doc(_S.valmentajaUid).update(pika);
        }
  ...
  function avaa(opts) {
    ...
    m.innerHTML = '<div onclick="event.stopPropagation()" style="' + _brandiCssVars(_O.brandi) + ';background:var(--bg2);border:.5px solid var(--border);border-radius:14px;width:min(560px,96vw);padding:20px 22px;margin:auto"><div id="haInner"></div></div>';
    document.body.appendChild(m);
    _render();
  }

  window.TM_HARJOITUS = { avaa: avaa, KRITEERIT_A: KRITEERIT_A, KRITEERIT_B: KRITEERIT_B, _autoIkavaihe: _autoIkavaihe, _brandiVars: _brandiVars, _kontrasti: _kontrasti };
})();
