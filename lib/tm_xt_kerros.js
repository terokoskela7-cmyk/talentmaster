...
function xtPiirraUhkakartta(svg, spec, asetukset) {
  ...
  var pelimuoto = spec && spec.pelimuoto;
  if (typeof xtProfiili === 'function' && xtProfiili(pelimuoto) === null) return null;   // esim. 3v3: ei karttaa
  ...
  for (var r = 0; r < XT_RIVIT; r++) {
    for (var c = 0; c < XT_SARAKKEET; c++) {
      ...
      // Arvo ruudun KESKIPISTEESTÄ pelimuodon profiililla (pienkentällä metrimuunnos, 11v11 = ruudukko suoraan).
      var v = xtArvo((b.x0 + b.x1) / 2, (b.y0 + b.y1) / 2, suunta, pelimuoto);
...
/* 3) RISKIKARTTA (käänteinen uhka). Näyttää MISSÄ pallonmenetys maksaa eniten: vastustajan xT samassa
   pisteessä. Punainen = korkea (oma rangaistusalue ja edusta), amber = kohonnut (oma puolustuskolmannes).
   Matalaa ei piirretä, jotta kartta ohjaa katseen vain olennaiseen. Piirretään kenttäviivojen alle. */
function xtPiirraRiskikartta(svg, spec) {
  var suunta = (spec && spec.suunta) || 'ylos', pelimuoto = spec && spec.pelimuoto;
  if (typeof xtProfiili === 'function' && xtProfiili(pelimuoto) === null) return null;
  var juuri = svg.querySelector('g') || svg;
  var g = _xtkEl('g', { 'class': 'xt-riskikartta', 'pointer-events': 'none' });
  for (var r = 0; r < XT_RIVIT; r++) {
    for (var c = 0; c < XT_SARAKKEET; c++) {
      var b = _xtkRuudunRajat(r, c, suunta);
      var mr = xtMenetysriski((b.x0 + b.x1) / 2, (b.y0 + b.y1) / 2, suunta, pelimuoto);
      if (!mr || mr.taso === 'matala') continue;
      g.appendChild(_xtkEl('rect', {
        x: _xtkPX(b.x0), y: _xtkPY(b.y0), width: _xtkPX(b.x1) - _xtkPX(b.x0), height: _xtkPY(b.y1) - _xtkPY(b.y0),
        fill: mr.taso === 'korkea' ? 'var(--red)' : 'var(--amber, #E0A040)',
        'fill-opacity': mr.taso === 'korkea' ? 0.30 : 0.16,
        stroke: 'var(--bg)', 'stroke-width': 0.6, 'stroke-opacity': 0.5, 'data-riski': mr.taso
      }));
    }
  }
  juuri.insertBefore(g, juuri.firstChild);
...
/* 4) MENETYKSET pelihavainnosta: merkki menetyspisteeseen (rengas + risti). Korkean riskin menetys
   punaisena, muut vaimeina. tapahtumat = xtHavaintoYhteenvedon syöte (vain onnistui:false piirretään). */
function xtPiirraMenetykset(svg, tapahtumat, suunta, pelimuoto) {
  var juuri = svg.querySelector('g') || svg;
  var g = _xtkEl('g', { 'class': 'xt-menetykset', 'pointer-events': 'none' });
  (tapahtumat || []).forEach(function (t, i) {
    if (!t || t.onnistui !== false) return;
    var mp = xtMenetyspiste(t); if (!mp) return;
    var mr = xtMenetysriski(mp.x, mp.y, suunta, pelimuoto);
    var korkea = mr && mr.taso === 'korkea';
    var vari = korkea ? 'var(--red)' : 'var(--ink3)';
    var x = _xtkPX(mp.x), y = _xtkPY(mp.y), k = 4.2;
    var m = _xtkEl('g', { 'class': 'xt-menetys', 'data-riski': mr ? mr.taso : '', 'data-indeksi': i });
    m.appendChild(_xtkEl('circle', { cx: x, cy: y, r: 7.5, fill: 'var(--bg)', 'fill-opacity': 0.85, stroke: vari, 'stroke-width': korkea ? 1.6 : 1 }));
    m.appendChild(_xtkEl('line', { x1: x - k, y1: y - k, x2: x + k, y2: y + k, stroke: vari, 'stroke-width': 1.6, 'stroke-linecap': 'round' }));
    m.appendChild(_xtkEl('line', { x1: x - k, y1: y + k, x2: x + k, y2: y - k, stroke: vari, 'stroke-width': 1.6, 'stroke-linecap': 'round' }));
    g.appendChild(m);
  });
  juuri.appendChild(g);
...
function xtPiirraKerros(svg, spec, asetukset) {
  ...
  // Järjestys: riskikartta ja uhkakartta molemmat kenttäviivojen alle (insertBefore firstChild),
  // riski viimeisenä → se jää alimmaksi eikä peitä uhkakarttaa, jos molemmat ovat päällä.
  if (o.uhkakartta !== false) xtPiirraUhkakartta(svg, spec, o);
  if (o.riski) xtPiirraRiskikartta(svg, spec);
...
var TM_XT_KERROS = {
  ...
  xtPiirraRiskikartta: xtPiirraRiskikartta, xtPiirraMenetykset: xtPiirraMenetykset
};
