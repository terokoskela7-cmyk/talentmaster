...
(function (root) {
  ...
  // nimi_sv (i18n V8e): puhdasta dataa nimen rinnalla — KIELIVALINTA TEHDÄÄN NÄKYMÄSSÄ (ctx.teemaNimi), ei täällä.
  // Termit konformoivat olemassa oleviin sv-karttoihin (Kestävyys→Uthållighet, Ketteryys→Smidighet) + lukittuun
  // glossaariin (Kehon valmius→Kroppslig beredskap, tm_i18n_common).
  var TM_FYYSTEEMAT = [
    { avain: 'fy_nopeus', nimi: 'Nopeus', nimi_sv: 'Snabbhet', ikoni: '🏃', testit: ['lin5m', 'lin10m', 'lin30m'] },
    { avain: 'fy_rajahtavyys', nimi: 'Räjähtävyys', nimi_sv: 'Explosivitet', ikoni: '⚡', testit: ['cmj', 'sj'] },
    { avain: 'fy_kestavyys', nimi: 'Kestävyys', nimi_sv: 'Uthållighet', ikoni: '🫁', testit: ['mas'] },
    { avain: 'fy_ketteryys', nimi: 'Ketteryys & suunnanmuutos', nimi_sv: 'Smidighet & riktningsförändring', ikoni: '🔄', testit: ['kasirata', 'sm_juoksu'] },
    { avain: 'fy_liikehallinta', nimi: 'Liikehallinta (kehon valmius)', nimi_sv: 'Rörelsekontroll (kroppslig beredskap)', ikoni: '🧘', testit: [], flei: true }   // evidenssi = FLEI (Teron päätös 2026-07-09)
  ];
  ...
  // i18n V8e — ctx (valinnaiset, oletukset = nykyinen fi → taaksepäin-yhteensopiva; lib pysyy kielineutraalina):
  //   ctx.teemaNimi: fn(teema)→nimi (näkymä valitsee nimi/nimi_sv) · ctx.osaindeksiNimi: fn(osaindeksiAvain)→nimi
  //   ctx.tekstit: { heikoinD1, kehonValmiusPre, kehonValmiusPost, taso } — peruste-templaatin kiinteät osat.
  // Numerot (pct, taso) aina verbatim. Rakenteiset osat palautetaan myös erikseen (peruste_osat) jotta kutsuja
  // voi halutessaan koostaa tekstin itse ilman merkkijonojen purkamista.
  function tmFyysEhdota(p, ctx) {
    if (!p) return null;
    ctx = ctx || {};
    var TXT = ctx.tekstit || {};
    var nimiFn = (typeof ctx.teemaNimi === 'function') ? ctx.teemaNimi : null;
    var oiNimiFn = (typeof ctx.osaindeksiNimi === 'function') ? ctx.osaindeksiNimi : null;
    var _tNimi = function (t, avain) { return (nimiFn && nimiFn(avain)) || (t ? t.nimi : avain); };
    var pct = _fleiPct(p.flei_viimeisin);
    if (pct != null && pct < FLEI_KLINIKKA) {
      var fl = tmFyysTeema('fy_liikehallinta');
      return { avain: fl.avain, nimi: _tNimi(fl, fl.avain), prioriteetti: true,
        peruste: (TXT.kehonValmiusPre || 'Kehon valmius ') + pct + (TXT.kehonValmiusPost || ' < 40 → klinikkajatkumo (§14)'),
        peruste_osat: { tyyppi: 'flei', pct: pct } };
    }
    if (ctx.osaindeksit) {
      var oi = tmFyysHeikoinOsaindeksi(ctx.osaindeksit);
      if (oi) {
        var t2 = tmFyysTeema(oi.teema);
        var tNimi = _tNimi(t2, oi.teema);
        var oiNimi = (oiNimiFn && oiNimiFn(oi.osaindeksi)) || oi.nimi;
        return { avain: oi.teema, nimi: tNimi, taso: oi.taso, osaindeksi: oi.osaindeksi,
          peruste: (TXT.heikoinD1 || 'Heikoin D1: ') + tNimi + ' (' + oiNimi + (TXT.taso || ' taso ') + oi.taso + ')',
          peruste_osat: { tyyppi: 'osaindeksi', teema: oi.teema, osaindeksi: oi.osaindeksi, taso: oi.taso } };
      }
    }
    ...
    if (testi && !D2_TESTIT[testi]) {
      ...
      if (tm) return { avain: tm.avain, nimi: _tNimi(tm, tm.avain), testi: testi,
        peruste: (TXT.heikoinD1 || 'Heikoin D1: ') + _tNimi(tm, tm.avain),
        peruste_osat: { tyyppi: 'testi', teema: tm.avain, testi: testi } };
    }
    return null;
  }
