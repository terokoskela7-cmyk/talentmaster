  ...
  function tmKehityskaari(el, data, opts) {
    ...
    if (taso !== 'lahtopiste') {
      if (om === 'tki' && !pelaaja) {
        ...
        var su = data.suunta || (typeof tmKaariSuunta === 'function' ? null : null);
        var dlt = (data.deltaAbs != null) ? ((Number(data.deltaAbs) > 0 ? '+' : '') + data.deltaAbs + (yks ? ' ' + yks : '')) : '';
        var up = (data.deltaAbs != null && Number(data.deltaAbs) > 0) || su === 'up';
    ...
    // Sparkline (≥3) TAI 2-pisteen viiva; ref-viiva TKI:lle (kaksi deltaa)
    ...
    if (taso !== 'lahtopiste') h += _kkSpark(arvot, pp, ref);
    else h += '<div style="font-family:var(--font-mono,monospace);font-size:9.5px;color:var(--ink3,#6B82A8);margin:8px 0">Yksi piste — toinen mittaus avaa kaaren.</div>';
