...
(function (root) {
  ...
  // K1b/K4 §22 — segmentoi alusta-herkkä sarja viimeisimmän eksplisiittisen alustan sisään VAIN kun ≥2 eri
  // eksplisiittistä alustaa (null = vertailukelpoinen → ei regressiota). Jaettu: _testiRivi (VP, merkin kanssa) +
  // tmKaariRenderPelaaja (§7.22, ILMAN merkkiä). Palauttaa { sarja, alustat, viimAlusta }; ei mutatoi alkuperäistä.
  function _alustaSegmentoi(avain, sarja) {
    var out = { sarja: sarja, alustat: [], viimAlusta: null };
    if (!ALUSTAHERKAT[avain] || !sarja || !sarja.length) return out;
    var expl = {};
    sarja.forEach(function (pt) { if (pt.alusta != null) expl[pt.alusta] = 1; });
    out.alustat = Object.keys(expl);
    if (out.alustat.length < 2) return out;
    for (var i = sarja.length - 1; i >= 0; i--) { if (sarja[i].alusta != null) { out.viimAlusta = sarja[i].alusta; break; } }
    var va = out.viimAlusta;
    out.sarja = sarja.filter(function (pt) { return pt.alusta == null || pt.alusta === va; });
    return out;
  }
  ...
  function _testiRivi(avain, sarja, ctx, opts) {
    ...
    if (ALUSTAHERKAT[avain] && sarja && sarja.length) {
      var _seg = _alustaSegmentoi(avain, sarja);
      if (_seg.alustat.length >= 2) {
        sarja = _seg.sarja;   // segmentoi viimeisimmän alustan sisään (jaettu _alustaSegmentoi)
        alustaMerkki = '<span style="color:var(--amber,#E0A040);font-size:9px;margin-left:4px" title="Eri alustan pisteet piilotettu — §22: vertailu vain saman alustan sisällä">⚠ ' + _esc(_alustaLyhyt(_seg.viimAlusta), ctx.esc) + '</span>';
      } else if (_seg.alustat.length === 1) {
        alustaMerkki = '<span style="color:var(--ink3);font-size:9px;margin-left:4px">· ' + _esc(_alustaLyhyt(_seg.alustat[0]), ctx.esc) + '</span>';
      } else {
  ...
  // K1a — FLEI (Kehon valmius · 0–100 · suurempi parempi) Kehityskaareen. Lähde flei_historia [{flei,pvm}] (pikakenttä,
  // §26 ei alikokoelmakyselyä). §7.22: EI pelaajapintaan (K4 hoitaa variantin) → vain tmKaariRenderFull. Datataso <2 → ''.
  function tmKaariFleiBlokki(p) {
    var hist = (p && Array.isArray(p.flei_historia))
      ? p.flei_historia.map(function (e) { return (e && e.flei != null) ? Number(e.flei) : null; }).filter(function (v) { return v != null && !isNaN(v); })
      : [];
    if (hist.length < 2) return '';
    var eka = hist[0], viim = hist[hist.length - 1];
    var vari = (viim > eka) ? 'var(--teal,#28B090)' : (viim < eka ? 'var(--amber,#E0A040)' : 'var(--ink3,#6B82A8)');
    var merkki = (viim > eka) ? '↑' : (viim < eka ? '↓' : '→');
    return '<div style="display:flex;align-items:center;gap:8px;font-size:11px;padding:3px 0">'
      + '<span style="min-width:96px;color:var(--ink2,#9AAAC4)">Kehon valmius · FLEI</span>'
      + '<span style="color:var(--ink3,#6B82A8);font-family:var(--font-m,monospace);font-size:10px">' + _fmt(eka) + '→' + _fmt(viim) + ' <span style="opacity:.7">(' + hist.length + ' mitt.)</span></span>'
      + '<span style="margin-left:auto;color:' + vari + ';font-weight:600">' + merkki + '</span></div>';
  }
  ...
  function tmKaariRenderFull(p, ctx) {
    ...
    var _fleiBlokki = tmKaariFleiBlokki(p);   // K1a — FLEI-kaarirvi (kooste-tasolla, suurempi parempi)
    ...
    if (!fyysR.length && !teknR.length && !aggR.length && !_adarBlokki && !_fleiBlokki) {
    ...
    return '<div style="padding:2px 0">' + rivit + aggHtml + _fleiBlokki
  ...
  function tmKaariRenderPelaaja(p, ctx) {
    ...
    avaimet.forEach(function (k) {
      var s = _alustaSegmentoi(k, tmKaariSarja(hh, k)).sarja;   // K4 §22 — segmentoi ennen suuntaa (ei valhe-parannusta sekava-alustalta)
      if (!tmKaariKattavuusOk(s)) return;
      var suunta = tmKaariSuunta(k, s);
      if (suunta.parani === true) parannukset.push({ k: k, s: s });
    });
    tkiAv.forEach(function (k) {
      var s = _alustaSegmentoi(k, tmKaariSarja(tki, k)).sarja;   // K4 §22 — sama vartija (syotto/pujottelu ovat alustaherkkiä)
      if (!tmKaariKattavuusOk(s)) return;
      var suunta = tmKaariSuunta(k, s);
      if (suunta.parani === true) parannukset.push({ k: k, s: s });
    });
    // K4 — FLEI "Kehon valmius" §7.22-variantti: vain kun parani (suurempi parempi); positiivinen verbi + sparkline.
    // EI lukuja/amber/normia — VP:n tmKaariFleiBlokki näyttää ne → EI §7.22-turvallinen pelaajapinnassa, ei kutsuta sitä.
    var fleiHist = (p && Array.isArray(p.flei_historia)) ? p.flei_historia.map(function (e) { return (e && e.flei != null) ? Number(e.flei) : null; }).filter(function (v) { return v != null && !isNaN(v); }) : [];
    if (fleiHist.length >= 2 && fleiHist[fleiHist.length - 1] > fleiHist[0]) {
      parannukset.push({ k: 'flei', s: fleiHist.map(function (v) { return { arvo: v }; }) });
    }
    var onDataa = tmKaariMitatutAvaimet(hh).some(function (k) { return tmKaariKattavuusOk(tmKaariSarja(hh, k)); })
      ...
      || fleiHist.length >= 2;   // K4 — FLEI-only-pelaaja: ≥2 mittausta = dataa (ei honest-empty-seed)
    ...
    var VERBI = { lin30m: '30 m nopeutui', lin10m: '10 m nopeutui', lin5m: '5 m nopeutui', kasirata: 'Ketteryys parani', sm_juoksu: 'Suunnanmuutos parani', sm_pallo: 'Pallonhallinta parani', cmj: 'Ponnistus parani', mas: 'Kestävyys parani', pujottelu: 'Pujottelu parani', syotto: 'Syöttö parani', tki: 'Tekniikka nousi', kokonaistulos: 'Tekniikka parani', flei: 'Kehon valmius parani' };
  ...
  var API = {
    ...
    tmKaariAdarBlokki: tmKaariAdarBlokki, tmKaariAdarDimensiot: tmKaariAdarDimensiot, tmKaariFleiBlokki: tmKaariFleiBlokki,   // K5a
    ...
    tmKaariSiru: _sparkline   // K3 — pieni sparkline-siru (sarja=[{arvo}] pvm-järj.); '' kun <2 pistettä
  };
  ...
  if (typeof window !== 'undefined') {
    ...
    root.tmKaariSiru = _sparkline;   // K3
    ...
    root.tmKaariAdarBlokki = tmKaariAdarBlokki; root.tmKaariAdarDimensiot = tmKaariAdarDimensiot; root.tmKaariFleiBlokki = tmKaariFleiBlokki;   // K5a
