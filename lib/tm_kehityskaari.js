...
(function (root) {
  ...
  function _testiRivi(avain, sarja, ctx) {
  ...
  function tmKaariRenderFull(p, ctx) {
    ...
    // kattavuus: onko yhdelläkään avaimella ≥2 pistettä
    var mitattavat = [];
    hhAvaimet.forEach(function (k) { var s = tmKaariSarja(hh, k); if (tmKaariKattavuusOk(s)) mitattavat.push({ k: k, s: s }); });
    tkiAvaimet.forEach(function (k) { var s = tmKaariSarja(tki, k); if (tmKaariKattavuusOk(s)) mitattavat.push({ k: k, s: s }); });
    if (!mitattavat.length) {
      return '<div style="font-size:11px;color:var(--ink3);padding:8px 0">Kehityskaari täyttyy kun mittauksia on ≥2.</div>';
    }
    var rivit = mitattavat.map(function (m) { return _testiRivi(m.k, m.s, ctx); }).join('');
    ...
    ['d1_taso', 'd2_taso'].forEach(function (tk) {
      ...
      tasoHtml += '<div style="display:flex;align-items:center;gap:8px;padding:3px 0;font-size:11px">'
        ...
        + '<span style="color:var(--ink3);font-family:var(--font-m,monospace);font-size:10px">taso ' + suunta.ensimmainen + '→' + suunta.viimeinen + '</span>'
    ...
    if (jaksot.length) {
      ...
      var rows = jaksot.slice(0, 4).map(function (j) {
        ...
        var vaikutus = sidos
          ? (' · ' + _esc(tmKaariNimi(key), ctx.esc) + ' ' + sidos.ennen + '→' + sidos.jalkeen + ' ' + (sidos.parani ? '✓' : '○'))
          : '';
    ...
    return '<div style="padding:2px 0">' + rivit
