...
/** { kolmannes: 'puolustus'|'keski'|'hyokkays', kaista: 'vasen_laita'|'keskusta'|'oikea_laita' } */
function phVyohyke(p) {
  ...
  return {
    ...
    kaista: wid < 100 / 3 ? 'vasen_laita' : (wid < 200 / 3 ? 'keskusta' : 'oikea_laita')
  };
}
...
function phArvo(m, pelimuoto) {
  ...
  switch (m.tyyppi) {
    ...
    case 'kaksinpeli':
      if (m.rooli === 'puolustus' && m.tulos === 'voitti') return puolustus(m.piste || m.alku);
      if (m.rooli === 'hyokkays' && m.tulos === 'ei_ohittanut') return riski(m.piste || m.alku);
      return null;
    case 'laukaus': {
      var xg = phXg(m.piste || m.loppu || m.alku, pelimuoto, (m.malli && m.malli.xg) || null);
...
/** Ketjun jasenet (viittaus juuren id:hen). */
function _phKetju(merkinnat, juuri) {
  if (!juuri || juuri.id == null) return [];
  return merkinnat.filter(function (x) { return x.ketju === juuri.id; });
}
...
function _phSiirtyma(merkinnat, pelimuoto) {
  ...
  merkinnat.filter(_phOnJuuri).forEach(function (juuri) {
    ...
    var eka = function (tyyppi) {
      for (var i = 0; i < jasenet.length; i++) { if (jasenet[i].tyyppi === tyyppi) return jasenet[i]; }
      return null;
    };
    ...
    if (jatko === 'syotto') {
      var s = eka('syotto');
      if (s && s.perilla !== false) { out.sailyi++; if (etenee(s)) out.eteniHeti++; }
      else { out.menetetty++; out.menetysHeti++; }
    } else if (jatko === 'kuljetus') {
      var k = eka('kuljetus');
      if (k && k.lopputuote !== 'menetys') { out.sailyi++; if (etenee(k)) out.eteniHeti++; }
      else { out.menetetty++; out.menetysHeti++; }
...
function phYhteenveto(dok) {
  ...
  var kaikki = merkinnat.filter(function (m) { return m && !m.eiSijaintia; });
  var ketjussa = function (m) { return m.ketju != null; };

  // 1v1 hyokkays: kaksinpelit + kuljetukset joissa ohitus matkalla.
  var hyokkays1v1 = merkinnat.filter(function (m) { return m.tyyppi === 'kaksinpeli' && m.rooli === 'hyokkays'; });
  var ohitusKuljetukset = merkinnat.filter(function (m) { return m.tyyppi === 'kuljetus' && m.ohitus === true; });
  var ykkosetHyokkays = {
    onnistui: hyokkays1v1.filter(function (m) { return m.tulos === 'ohitti' || m.tulos === 'rikottiin'; }).length + ohitusKuljetukset.length,
    yritykset: hyokkays1v1.length + ohitusKuljetukset.length
  };

  var puolustus1v1 = merkinnat.filter(function (m) { return m.tyyppi === 'kaksinpeli' && m.rooli === 'puolustus'; });
  ...
  uhkaMerkinnat.forEach(function (m) {
    ...
    luotuUhka += p;
    if (ketjussa(m)) {
      var juuri = merkinnat.filter(function (x) { return x.id === m.ketju; })[0];
