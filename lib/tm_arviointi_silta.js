...
(function (root) {
  ...
  //   palloliittoNimi: fn(palloliitto_avain)→nimi (i18n V8e: taksonomian nimi_sv); oletus = SILTA_NIMET (fi).
  //   tekstit: { heikoin, havaittu } — syy-prefiksit (i18n V8e); oletus fi. KIELIVALINTA TEHDÄÄN KUTSUJASSA,
  //     tämä lib pysyy kielineutraalina (sama kuvio kuin konseptiNimi). Numerot ("(2/5)") aina verbatim.
  ...
  function tmSiltaEhdota(havaittu, ctx) {
    ...
    var pkNimiFn = (typeof ctx.palloliittoNimi === 'function') ? ctx.palloliittoNimi : null;
    var TXT = ctx.tekstit || {};
    var _pkNimi = function (pk) { return (pkNimiFn && pkNimiFn(pk)) || SILTA_NIMET[pk] || pk; };
    var kand = [];
    ...
    return kand.slice(0, 3).map(function (x, i) {
      return {
        ...
        palloliitto_nimi: _pkNimi(x.pk),
        arvo: x.arvo,
        konsepti_avain: x.ka,
        konsepti_nimi: nimiFn ? (nimiFn(x.ka) || x.ka) : x.ka,
        syy: (i === 0 ? (TXT.heikoin || 'Heikoin havaittu: ') : (TXT.havaittu || 'Havaittu: ')) + _pkNimi(x.pk) + ' (' + x.arvo + '/5)'
      };
    });
  }
