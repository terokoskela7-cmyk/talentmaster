...
(function (root) {
  ...
  // Jakson etenemä: monesko viikko menossa ja montako jäljellä. { n, k, jaljella } | null.
  // Sama sääntö kuin tmJfUmpeutunut: ei alkoi-pvm:ää (tai virheellinen) → null, EI arvausta.
  // n on 1-pohjainen ja rajattu välille 1..k, jotta "Viikko n/k" ei koskaan näytä 0/k tai 9/8.
  function tmJfViikko(jf, nyt) {
    if (!jf || !jf.alkoi) return null;
    var alkoi = new Date(jf.alkoi).getTime();
    if (isNaN(alkoi)) return null;
    var k = jf.kesto_vk || 4;
    var kulunut = Math.floor((_nowMs(nyt) - alkoi) / (7 * DAY_MS)) + 1;
    var n = Math.max(1, Math.min(k, kulunut));
    return { n: n, k: k, jaljella: Math.max(0, k - n) };
  }
