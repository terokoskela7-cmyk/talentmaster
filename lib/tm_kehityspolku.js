...
(function (root) {
  ...
  function tmRatkaiseKehityspolku(idpFokus, pelaaja, ctx) {
    ctx = ctx || {};
    // i18n V8e: syy-tekstit kutsujasta (oletus fi) — lib pysyy kielineutraalina, kuten konseptiNimi.
    // Konseptinimet (_nimi) ovat teknis-taktista curriculumia (Tier 2) → jäävät fi:ksi kunnes se lib käännetään.
    var TXT = ctx.tekstit || {};
    if (!idpFokus || !idpFokus.alue) {
      return { tyyppi: 'ei_konseptia', alue: null, dim: null, konsepti_avain: null, konsepti_nimi: null,
        ...
        cue: [], harjoite: [], varmuus: 'ei_konseptia', syy: TXT.eiTavoitetta || 'Ei aktiivista tavoitetta.' };
    }
    ...
    if (TULOSMITTARI[alue]) {
      return { tyyppi: 'tulosmittari', alue: alue, dim: dim, konsepti_avain: null, konsepti_nimi: null,
        ...
        syy: TXT.tulosmittari || 'Seuraus, ei harjoiteltava taito — kehitä alla olevia taitoja + päätöksenteko (D4).' };
    }
    ...
    if (MODIFIERI[alue]) {
      return { tyyppi: 'modifieri', alue: alue, dim: dim, konsepti_avain: null, konsepti_nimi: null,
        ...
        syy: TXT.modifieri || 'Ei erillinen konsepti — harjoittele heikommalla jalalla muissa konsepteissa.' };
    }
    ...
    if (D2_KONSEPTI[alue] || dim === 'D2') {
      ...
      if (!konsepti) {
        return { tyyppi: 'teknis_taktinen', alue: alue, dim: 'D2', konsepti_avain: null, konsepti_nimi: null,
          fundamentti_avain: null, fundamentti_nimi: null, harjoite_tagit: null, havainnointi_vihje: false,
          cue: [], harjoite: [], varmuus: 'ei_konseptia', syy: TXT.eiD2Konseptia || 'D2-kohteelle ei kartoitettua konseptia.' };
      }
      return _teknisTulos(alue, konsepti, 'D2', pelaaja, ctx, 'lukittu',
        { harjoite_tagit: HARJOITE_TAGIT[alue] || null, alt: ALT_VIHJE[alue] || null,
          syy: (TXT.ehdotettuPre || 'Tavoitteesta ehdotettu: ') + _nimi(konsepti, ctx) + '.' });
    }
    if (dim === 'D4') {
      ...
      if (typeof ctx.tmPelialyYksiloEhdota === 'function') {
        var ehd = ctx.tmPelialyYksiloEhdota([alue], { sallitutKonseptit: ctx.sallitutKonseptit, konseptiNimi: ctx.konseptiNimi,
          tekstit: ctx.tekstit, taksonomiaNimi: ctx.taksonomiaNimi }) || [];   // i18n V8e: kieli välittyy ketjun läpi
        if (ehd.length) top = ehd[0];
      }
      if (top && top.konsepti_avain) {
        var r = _teknisTulos(alue, top.konsepti_avain, 'D4', pelaaja, ctx, 'ehdotettu',
          { syy: top.syy || ((TXT.ehdotettuPre || 'Tavoitteesta ehdotettu: ') + _nimi(top.konsepti_avain, ctx) + '.') });
        return r;
      }
      return { tyyppi: 'teknis_taktinen', alue: alue, dim: 'D4', konsepti_avain: null, konsepti_nimi: null,
        fundamentti_avain: null, fundamentti_nimi: null, harjoite_tagit: null, havainnointi_vihje: false,
        cue: [], harjoite: [], varmuus: 'ei_konseptia', syy: TXT.eiPelialyKarttaa || 'Peliäly-kohteelle ei konseptikarttaa (KARTTA_A puuttuu ctx:stä).' };
    }
    if (dim === 'D1') {
      return { tyyppi: 'fyysinen', alue: alue, dim: 'D1', konsepti_avain: null, konsepti_nimi: null,
        ...
        syy: TXT.fyysinen || 'Fyysinen kehityskohde — reititä fyysiseen jaksoteemaan (ei teknis-taktista harjoitetta).' };
    }
    if (dim === 'D3' || dim === 'D5') {
      return { tyyppi: 'laadullinen', alue: alue, dim: dim, konsepti_avain: null, konsepti_nimi: null,
        fundamentti_avain: null, fundamentti_nimi: null, harjoite_tagit: null, havainnointi_vihje: false,
        cue: [], harjoite: [], varmuus: 'ei_konseptia',
        syy: (dim === 'D3' ? (TXT.henkinen || 'Henkinen kehityskohde — kehityskeskustelu/rooli, ei pakotettua harjoitetta.')
                            : (TXT.sosiaalinen || 'Sosiaalinen kehityskohde — kehityskeskustelu/rooli, ei pakotettua harjoitetta.')) };
    }
    // Tuntematon dim / ei kartoitettu.
    return { tyyppi: 'ei_konseptia', alue: alue, dim: dim, konsepti_avain: null, konsepti_nimi: null,
      fundamentti_avain: null, fundamentti_nimi: null, harjoite_tagit: null, havainnointi_vihje: false,
      cue: [], harjoite: [], varmuus: 'ei_konseptia', syy: TXT.eiPolkua || 'Kohteelle ei kartoitettua kehityspolkua.' };
  }
