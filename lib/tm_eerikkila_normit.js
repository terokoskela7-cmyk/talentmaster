...
//   rivi = { label, arvo, yks, taso, asteikko:3|5, lahde:'hh'|'tk', neutraali, xfactor }
...
// - §28: phv_tila ∈ {PRE,LAH} TAI puuttuu → lin30m/mas/cmj saavat neutraali:true (ei kehityskohde). 5m/10m/syöttö/pujottelu/TK eivät.
...
function perTestTasot(p, ika, sp) {
  ...
  var prePhv = (p.phv_tila == null || p.phv_tila === '' || p.phv_tila === 'PRE' || p.phv_tila === 'LAH');
  var NEUTR = { lin30m: 1, mas: 1, cmj: 1 };   // §28 post-PHV-herkät
  var out = [];
  var lisaaEer = function (key, arvo, eer, label, yks, asteikko, kmh) {
    if (arvo == null || isNaN(arvo)) return;
    var taso = null;
    if (ika != null && spEer) { var tt = eerikkilaTaso(kmh ? (+arvo / 3.6) : +arvo, eer, ika, spEer); taso = (tt > 0) ? tt : null; }
    out.push({ label: label, arvo: +arvo, yks: yks, taso: taso, asteikko: asteikko, lahde: 'hh',
      neutraali: !!(NEUTR[key] && prePhv), xfactor: (asteikko === 3 ? taso === 3 : taso === 5) });
  };
  // 1) Fyysiset (Eerikkilä 5-port). hyppy_sj:lle ei normia → taso jää nulliksi (raaka).
  lisaaEer('lin5m',  hv.lin5m,  'nopeus_5m',  '5m',  's', 5);
  lisaaEer('lin10m', hv.lin10m, 'nopeus_10m', '10m', 's', 5);
  lisaaEer('lin30m', hv.lin30m, 'nopeus_30m', '30m', 's', 5);
  lisaaEer('cmj',    hv.cmj,    'hyppy_cj',   'CMJ', 'cm', 5);
  lisaaEer('sj',     hv.sj,     'hyppy_sj',   'SJ',  'cm', 5);
  lisaaEer('mas',    hv.mas,    'mas',        'MAS', 'km/h', 5, true);
  lisaaEer('kasirata', hv.kasirata, 'kasirata', 'Kasirata', 's', 5);
  lisaaEer('sm_juoksu', p.sm_juoksu_viimeisin, 'sm_juoksu', 'SM-juoksu', 's', 5);
  lisaaEer('sm_pallo',  p.sm_pallo_viimeisin,  'sm_pallo',  'SM-pallo',  's', 5);
  // 2) H-H tekniikka (3-port)
  lisaaEer('syotto',    hv.syotto,    'syotto',    'Syöttö',    's', 3);
  lisaaEer('pujottelu', hv.pujottelu, 'pujottelu', 'Pujottelu', 's', 3);
  ...
  if (tkl && typeof tkLajiTaso === 'function') {
    ...
    [['syotto_s', 'syotto', 'Syöttö (TK)'], ['pujottelu_s', 'pujottelu', 'Pujottelu (TK)'],
     ['ponnauttelu_s', 'ponnauttelu', 'Ponnauttelu (TK)'], ['kuljetus_laukaus_s', 'kuljetus_laukaus', 'Kuljetus-laukaus (TK)']
      ...
      out.push({ label: m[2], arvo: +a, yks: 's', taso: taso, asteikko: 5, lahde: 'tk', neutraali: false, xfactor: (taso === 5) });
    });
  }
  return out;
}
