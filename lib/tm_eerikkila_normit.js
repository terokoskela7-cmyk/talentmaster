 ...
 */
function eerikkilaProfiilit(pelaaja) {
  const ika   = pelaaja.syntymaVuosi
    ? new Date().getFullYear() - pelaaja.syntymaVuosi : null;
  const sukup = pelaaja.sukupuoli || 'M';
  if (!ika) return {};

  const testit = [
    ['nopeus_5m',    pelaaja.huhe_5m_s],
    ['nopeus_10m',   pelaaja.huhe_10m_s],
    ['nopeus_20m',   pelaaja.huhe_20m_s],
    ['nopeus_30m',   pelaaja.huhe_30m_s],
    ['kasirata',     pelaaja.huhe_kasirata_s],
    ['sm_juoksu',    pelaaja.huhe_smjuoksu_s],
    ['sm_pallo',     pelaaja.huhe_smpallo_s],
    ['hyppy_cj',     pelaaja.huhe_cj_cm],
    ['mas',          pelaaja.huhe_mas_ms],
    ['pujottelu',    pelaaja.tekniikka_pujottelu_s],
    ['syotto',       pelaaja.tekniikka_syotto_s],
  ];

  const tulos = {};
  for (const [testi, arvo] of testit) {
    if (arvo != null) {
      tulos[testi] = eerikkilaTaso(arvo, testi, ika, sukup);
    }
  }
  return tulos;
}
