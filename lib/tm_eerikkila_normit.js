...
function laskeD1Osaindeksit(hh, ika, sp) {
  ...
  return { kiihdytys: ka([tasoT('lin5m'), tasoT('lin10m')]), maksinopeus: tasoT('lin30m'),
           voima: ka([tasoT('cmj'), tasoT('sj')]), ketteryys: tasoT('kasirata'), aerobinen: tasoT('mas') };
}
...
function vanhempiRaporttiTekstit(p, ika, sp) {
  ...
  // SEURAAVA ASKEL — positiivinen + §28, EI tasolukua/"heikkous"
  var seuraavaAskel, tukiKey = null;
  if (p.tki_kehityskohde && _VANH_LAJI[p.tki_kehityskohde]) {
    tukiKey = String(p.tki_kehityskohde).replace('_bonus', '');
    var ln = _VANH_LAJI[p.tki_kehityskohde];
    seuraavaAskel = { kohde: ln, teksti: 'Seuraava askel: ' + ln + '. Pieni parannus joka treenissä riittää' + (nuori ? ' — ja moni taito kehittyy kasvun myötä.' : '.') };
  } else if (heikoinOsa) {
    tukiKey = heikoinOsa;
    var on = _VANH_OSA[heikoinOsa];
    seuraavaAskel = { kohde: on, teksti: 'Seuraava askel: ' + on + '. Tämä vahvistuu leikkien' + (nuori ? ' ja kehittyy kasvun myötä — ei kiirettä.' : '.') };
  } else {
    seuraavaAskel = { kohde: null, teksti: 'Monipuolinen liikkuminen ja pelaaminen vievät eteenpäin — anna lapsen kokeilla eri lajeja.' };
  }

  // TUKIVINKIT — kehityskohteen täsmätoimi + pysyvä autonomiavinkki
  var tukivinkit = [];
  ...
  return { vahvuus: vahvuus, seuraavaAskel: seuraavaAskel, tukivinkit: tukivinkit, prosessikehu: prosessikehu };
}
