...
function tavoiteRadarAkselit(pelaajat) {
  pelaajat = pelaajat || [];
  var onHH = pelaajat.some(function (p) { return p && p.hh_viimeisin && Object.keys(p.hh_viimeisin).length; });
  var onTK = pelaajat.some(function (p) { return p && p.tk_lajit_viimeisin; });
  var tyyppi = onHH ? 'hh' : (onTK ? 'tk' : null);
  if (!tyyppi) return null;
  var kand = (tyyppi === 'hh') ? _RADAR_HH_AKSELIT : _RADAR_TK_AKSELIT;
  var src = (tyyppi === 'hh') ? 'hh_viimeisin' : 'tk_lajit_viimeisin';
  var akselit = [];
  ...
  if (akselit.length < 3) return null;   // collapse — liian harva akselijoukko
  return { tyyppi: tyyppi, akselit: akselit };
}
