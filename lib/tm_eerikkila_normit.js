...
function perTestTasot(p, ika, sp) {
  ...
  var lisaaEer = function (key, arvo, eer, label, yks, asteikko, dimensio, kmh) {
    ...
    out.push({ label: label, arvo: +arvo, yks: yks, taso: taso, asteikko: asteikko, lahde: 'hh', dimensio: dimensio,
  ...
  if (tkl && typeof tkLajiTaso === 'function') {
    ...
    [['syotto_s', 'syotto', 'Syöttö (TK)'], ['pujottelu_s', 'pujottelu', 'Pujottelu (TK)'],
     ['ponnauttelu_s', 'ponnauttelu', 'Ponnauttelu (TK)'], ['kuljetus_laukaus_s', 'kuljetus_laukaus', 'Kuljetus-laukaus (TK)']
      ...
      out.push({ label: m[2], arvo: +a, yks: 's', taso: taso, asteikko: 5, lahde: 'tk', dimensio: 'D2', neutraali: false, xfactor: (taso === 5), pienempi: true, seuraavaTaso: null, gap: null });
    });
  }
  return out;
}
...
function renderKehityskorttiHTML(p, ika, sp) {
  ...
  var rivi = function (r) {
    ...
    return '<div style="display:flex;align-items:center;gap:10px;padding:5px 0;border-top:' + bo + '">'
      + '<span style="flex:0 0 76px;color:' + T.ink2 + ';font-size:12px">' + r.label + src + '</span>'
      ...
      + '<span style="flex:1;min-width:0">' + vihje + '</span>'
  ...
  var kortti = function (lbl, val, capt, vari) {
    return '<div style="flex:1;background:var(--bg2);border:' + bo + ';border-radius:8px;padding:11px 13px">'
      + '<div style="font-size:10px;letter-spacing:.06em;color:' + T.ink3 + ';text-transform:uppercase">' + lbl + '</div>'
  ...
  var h = '';
  if (d1v != null || d2v != null) h += '<div style="display:flex;gap:10px;margin-bottom:2px">' + kortti('D1 Fyysinen', d1v, d1c, d1col) + kortti('D2 Tekninen', d2v, d2c, d2col) + '</div>';
  h += ryhma('Fyysinen (D1)', 'D1');
  h += ryhma('Tekniikka (D2)', 'D2');
  if (rivit.length) h += '<div style="margin-top:8px;font-size:10px;color:' + T.ink3 + ';line-height:1.5">🌱 = ennen kasvupyrähdystä, fyysistä ei vielä arvioida · ⭐ = X-Factor (huipputaso) · tekniikka 1–3, fyysinen 1–5</div>';
  return h;
}
