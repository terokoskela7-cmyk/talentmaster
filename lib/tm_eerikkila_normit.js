...
function laskeD1Osaindeksit(hh, ika, sp) {
  ...
  // Ketteryys (LL, kasirata) ja Suunnanmuutos (DIAG, sm_juoksu) eriytetty (§30): kaksi eri ominaisuutta, ei litistetä
  // yhteen keskiarvoon → valmentaja näkee kumpi on vahva/heikko. sm_pallo = D2/TSI (ei tähän). ka-apuri jää kiihdytys/voimalle.
  return { kiihdytys: ka([tasoT('lin5m'), tasoT('lin10m')]), maksinopeus: tasoT('lin30m'),
           voima: ka([tasoT('cmj'), tasoT('sj')]), ketteryys: tasoT('kasirata'), suunnanmuutos: tasoT('sm_juoksu'), aerobinen: tasoT('mas') };
}
...
// §28 kypsyystila gated-fyysiselle (30m/CMJ/MAS) — KOLME tilaa kun heikko (taso<3), muuten aina 'normaali':
//   'kasvu'    = mitattu/pääteltävä pre-PHV → heikkous odotettua (🌱 "kehittyy PHV:n jälkeen").
//   'epavarma' = PHV-dataa EI ole → emme tiedä pre/post → himmennetty "kypsyysdataa puuttuu" (EI punainen, EI 🌱-lupaus).
//   'normaali' = post-PHV (PH/POST/AN) TAI taso≥3 TAI ei-gated → heikkous on aito signaali (hhTasoVari).
// Puhdas primitiiveistä (Vitest); preOverride = onNeutraaliPrePHV(p):n ikäpohjainen pre-päättely (friv välittää).
function kypsyysTila(phvTila, taso, gated, preOverride) {
  var heikko = gated && taso != null && taso < 3;
  if (!heikko) return 'normaali';
  var pre = preOverride === true || phvTila === 'PRE' || phvTila === 'LAH';
  if (pre) return 'kasvu';
  if (!phvTila) return 'epavarma';   // kypsyys tuntematon → neutraali/epävarma, ei väitettä suuntaan
  return 'normaali';                 // PH/POST/AN → aito heikkous
}
...
// Kohortti-valitsin (#76, PÄÄTÖS: komposiitti kokonaistaso). Järjestä _tasoLvl:llä (max{d1,hh,d2,laskeD2Joustava})
// laskevasti → VAKAA talenttiydin (sama ranking riippumatta katsotusta ominaisuudesta, seurattavissa yli kausien).
// Rankaamattomat (ei tasoa) jätetään pois ydin-kohorteista. 'kaikki' → koko roster (myös rankaamattomat).
// kohortti: 'paras'→1 · 'top5'→5 · 'top10'→10 · 'kaikki'→kaikki. Palauttaa N parasta (tai vähemmän jos ei riitä).
function valitseKohortti(pelaajat, kohortti) {
  pelaajat = pelaajat || [];
  if (kohortti === 'kaikki' || !kohortti) return pelaajat.slice();
  var rankatut = pelaajat.map(function (p) { return { p: p, lvl: _tasoLvl(p) }; })
    .filter(function (x) { return x.lvl != null; })
    .sort(function (a, b) { return b.lvl - a.lvl; });
  var N = (kohortti === 'paras') ? 1 : (kohortti === 'top5') ? 5 : (kohortti === 'top10') ? 10 : rankatut.length;
  return rankatut.slice(0, N).map(function (x) { return x.p; });
}
...
// Per-testi-radarin akselivalinta + joukkueen raaka-ka (#73, MyEWay-pariteetti). Data-adaptiivinen:
// H-H-akselit jos hh_viimeisin-dataa, muuten TK-lajiakselit jos tk_lajit_viimeisin-dataa. Vain akselit joilla ≥1 mittaus.
// Palauttaa {tyyppi:'hh'|'tk', akselit:[{kentta,laji,key,raakaKa,n}]} | null kun <3 akselia (collapse, kuten 5D-radar).
// Raaka-ka:sta taso 1–5 johdetaan näkymässä (eerikkilaTaso/tkLajiTaso) — tämä funktio pysyy puhtaana (ei normiriippuvuutta).
var _RADAR_HH_AKSELIT = [
  { kentta: 'lin5m', key: '5m' }, { kentta: 'lin10m', key: '10m' }, { kentta: 'lin30m', key: '30m' },
  { kentta: 'sm_juoksu', key: 'SM-juoksu' }, { kentta: 'sm_pallo', key: 'SM-pallo' },
  { kentta: 'syotto', key: 'Syöttö' }, { kentta: 'pujottelu', key: 'Pujottelu' },
  { kentta: 'cmj', key: 'CMJ' }, { kentta: 'mas', key: 'MAS' }, { kentta: 'kasirata', key: 'Ketteryys' }
];
// kentta = tk_lajit_viimeisin-pikakentän avain (_s-suffiksi, §26) · laji = tkLajiTaso-laji-id (ilman _s).
// Pituuspotku pois: sen pikakenttä on pituuspotku_bonus_s (eri semantiikka) — 4 akselia riittää radariin.
var _RADAR_TK_AKSELIT = [
  { kentta: 'ponnauttelu_s',      laji: 'ponnauttelu',      key: 'Ponn.' },
  { kentta: 'syotto_s',           laji: 'syotto',           key: 'Syöttö' },
  { kentta: 'pujottelu_s',        laji: 'pujottelu',        key: 'Pujottelu' },
  { kentta: 'kuljetus_laukaus_s', laji: 'kuljetus_laukaus', key: 'Kulj-lauk.' }
];
// Yksi patteristo → akselit joilla ≥1 mittaus (raaka-ka + n). Erotettu, koska molemmat lasketaan sekadatalle.
function _radarAkselit(pelaajat, kand, src) {
  var akselit = [];
  kand.forEach(function (a) {
    var s = 0, c = 0;
    pelaajat.forEach(function (p) {
      var o = p && p[src]; var v = o ? o[a.kentta] : null;
      if (v != null && !isNaN(v)) { s += Number(v); c++; }
    });
    if (c > 0) akselit.push({ kentta: a.kentta, laji: a.laji || null, key: a.key, raakaKa: Math.round(s / c * 100) / 100, n: c });
  });
  return akselit;
}
// Kumpi radar-patteristo yltää ≥3 akseliin KOKO joukkueella (tyyppivalinta erotettu datasta → kohortti-vakautus).
// Käytä VP:ssä kerran per joukkue (ei per kohortti) → radar-tyyppi ei enää vaihdu Paras/Top-5/Koko-joukkue-valinnasta.
function radarPatteristotSaatavilla(fullTeam) {
  fullTeam = fullTeam || [];
  return {
    hh: _radarAkselit(fullTeam, _RADAR_HH_AKSELIT, 'hh_viimeisin').length >= 3,
    tk: _radarAkselit(fullTeam, _RADAR_TK_AKSELIT, 'tk_lajit_viimeisin').length >= 3
  };
}
// tyyppiPakotettu ('hh'|'tk'): palauta sen patteriston akselit annetusta pelaajajoukosta (kohortti) ilman auto-valintaa
// — ≥1 akseli → palauta, muuten null (kohortilla ei valitun patteriston dataa → näkymä näyttää siistin tyhjän).
// Ilman parametria = nykyinen auto (rikkain ≥3 voittaa, tasan → 'hh'); taaksepäin-yhteensopiva.
function tavoiteRadarAkselit(pelaajat, tyyppiPakotettu) {
  pelaajat = pelaajat || [];
  if (tyyppiPakotettu === 'hh' || tyyppiPakotettu === 'tk') {
    var kand = (tyyppiPakotettu === 'hh') ? _RADAR_HH_AKSELIT : _RADAR_TK_AKSELIT;
    var src = (tyyppiPakotettu === 'hh') ? 'hh_viimeisin' : 'tk_lajit_viimeisin';
    var pak = _radarAkselit(pelaajat, kand, src);
    return pak.length ? { tyyppi: tyyppiPakotettu, akselit: pak } : null;
  }
  // SEKADATA (Sibbo T-joukkueet: rikas TKI kaikilla + harva H-H osalla): laske akselit MOLEMMILLE patteristoille
  // ja valitse RIKKAIN (eniten akseleita) joka yltää ≥3:een. Tasan → 'hh' (fyysinen primaari).
  // Aiemmin 'hh' voitti jos yksikin pelaaja oli H-H-testattu → harva H-H (2 akselia) ohitti rikkaan TK:n → radar collapse.
  var hh = _radarAkselit(pelaajat, _RADAR_HH_AKSELIT, 'hh_viimeisin');
  var tk = _radarAkselit(pelaajat, _RADAR_TK_AKSELIT, 'tk_lajit_viimeisin');
  var hhOk = hh.length >= 3, tkOk = tk.length >= 3;
  var tyyppi;
  if (hhOk && tkOk) tyyppi = (hh.length >= tk.length) ? 'hh' : 'tk';   // rikkain; tasan → hh
  else if (hhOk) tyyppi = 'hh';
  else if (tkOk) tyyppi = 'tk';
  else return null;   // kumpikaan ei yllä ≥3 akseliin → collapse (kuten 5D-radar)
  return { tyyppi: tyyppi, akselit: (tyyppi === 'hh') ? hh : tk };
}

// Painopiste (#75): ominaisuus jolla suurin etäisyys tavoitetasoon (kynnys, default 3). Vain mitatut (ka != null).
// Palauttaa {nimi, ka, gap} (suurin gap = pienin ka < kynnys) | null kun kaikki tavoitteessa / ei dataa. §28-neutraali (ei punaista).
function painopisteOminaisuus(ominaisuudet, kynnys) {
  kynnys = (kynnys == null) ? 3 : kynnys;
  var alle = (ominaisuudet || []).filter(function (o) { return o && o.ka != null && !isNaN(o.ka) && o.ka < kynnys; });
  if (!alle.length) return null;
  alle.sort(function (a, b) { return a.ka - b.ka; });   // pienin ka = suurin gap ensin
  var o = alle[0];
  return { nimi: o.nimi, ka: o.ka, gap: Math.round((kynnys - o.ka) * 10) / 10 };
}

// Datapolku-kattavuusvajeet (#75): tyhjä tila → toimenpide. counts = {pelihavainto, suostumus, phv} (kpl) / n.
// Palauttaa vain ne joiden kattavuus < 100% (suurin vaje ensin) → caller renderöi CTA:t. Ei labeleja (UI päättää).
function kattavuusVajeet(counts, n) {
  counts = counts || {};
  if (!n) return [];
  var out = [];
  ['pelihavainto', 'suostumus', 'phv'].forEach(function (k) {
    var x = counts[k] || 0;
    var pct = Math.round(x / n * 100);
    if (pct < 100) out.push({ avain: k, x: x, n: n, pct: pct });
  });
  out.sort(function (a, b) { return a.pct - b.pct; });   // suurin vaje ensin
  return out;
}
...
// #69: täydennä hh_viimeisin sm_juoksu/sm_pallo-pikakentistä jos ne puuttuvat hh_viimeisin:istä.
// Persistoi recalcHH:ssa → Suunnanmuutos-rivi (laskeD1Osaindeksit) + per-testi-radar (SM-akselit) näkevät ne ilman
// re-importtia. Palauttaa TÄYDEN uuden hv-objektin jos jokin lisättiin, muuten null (= ei kirjoitusta).
function taydennaHvSm(hv, smJuoksuViimeisin, smPalloViimeisin) {
  hv = hv || {};
  var lisaaJ = (hv.sm_juoksu == null && smJuoksuViimeisin != null);
  var lisaaP = (hv.sm_pallo == null && smPalloViimeisin != null);
  if (!lisaaJ && !lisaaP) return null;
  var uusi = Object.assign({}, hv);
  if (lisaaJ) uusi.sm_juoksu = smJuoksuViimeisin;
  if (lisaaP) uusi.sm_pallo = smPalloViimeisin;
  return uusi;
}
...
// §26-signaali: pelaaja teki D3-itsearvion mutta valmentaja ei vielä → kalibrointi kesken (kutsu VP/valmentajalle).
// Poistuu automaattisesti kun d3_varmuus muuttuu 'trianguloitu':ksi (molemmat lähteet). Pelkkä pikakenttä, ei kyselyä.
function d3OdottaaKalibrointia(p) {
  return !!(p && p.d3_varmuus === 'itsearvio');
}
var D3_VARMUUS_META = {
...
function perTestTasot(p, ika, sp) {
  ...
  lisaaEer('kasirata', hv.kasirata, 'kasirata', 'Ketteryys', 's', 5, 'D1');
...
// Testiarvon esitys (Osa F): raaka-arvot voivat olla ylitarkkoja (5.227 s). Yksikkökohtainen pyöristys VAIN NÄYTTÖÖN
// (ei muuta tallennettua raakadataa). s → 2 des · cm → 0–1 des · km/h → 1 des · m → 0 des · muu → 2 des.
// Math.round-pohjainen → ei float-artefakteja; String() pudottaa turhat perännollat (30.0 → "30").
function _fmtTestiArvo(arvo, yks) {
  if (arvo == null || arvo === '' || isNaN(arvo)) return '';
  var n = Number(arvo);
  var pyor = function (d) { var f = Math.pow(10, d); return String(Math.round(n * f) / f); };
  switch (String(yks || '')) {
    case 's': return pyor(2);
    case 'cm': return pyor(1);
    case 'km/h': return pyor(1);
    case 'm': return pyor(0);
    default: return pyor(2);
  }
}
...
function renderKehityskorttiHTML(p, ika, sp) {
  ...
  var tasoVari = function (r) {
    ...
    if (r.neutraali && r.taso <= 3) return T.ink3;                       // §28: kypsyys huomioitu → harmaa (EI teal/punainen), ei rankaisu
  ...
  var rivi = function (r) {
    ...
    // §28 (KYPSYYS_LUKU_NAKYVIIN-yhtenäisyys): näytä taso AINA harmaana (ink3) + 🌱 — ei enää "ei vielä arvioida".
    // 🌱 = kypsyys huomioitu, ei rankaise (ei punaista, ei gap-vihjettä).
    else if (r.neutraali && r.taso <= 3) badge = '<span style="display:inline-block;min-width:46px;text-align:center;padding:2px 8px;border-radius:4px;border:.5px solid ' + T.ink3 + ';color:' + T.ink3 + ';background:rgba(255,255,255,.03);font-size:11px;font-weight:600;white-space:nowrap">🌱 ' + r.taso + ' / ' + r.asteikko + '</span>';
    ...
    return '<div style="display:flex;flex-wrap:wrap;align-items:center;gap:4px 10px;padding:6px 0;border-top:' + bo + '">'
      ...
      + '<span style="flex:0 0 62px;font-family:\'Cormorant Garamond\',serif;font-size:15px;color:' + T.ink + '">' + _fmtTestiArvo(r.arvo, r.yks) + '<span style="font-size:10px;color:' + T.ink3 + '"> ' + r.yks + '</span></span>'
  ...
  // D2-kortti: per-laji d2_taso ENSIN (kanoninen 1–5), TKI/20 vain fallback (sama kaanon kuin VP laskeD2Taso, PR #262).
  // Lineaarinen TKI/20 ei vastaa per-laji-Eerikkilä-asteikkoa (esim. TKI 34 → 1.7 vs per-laji d2_taso 4) → ristiriita
  // per-laji-erittelyn kanssa. TKI näkyy edelleen omana indeksinään. Lähde-merkintä §30.
  var d2v = null, d2c = '—', d2col = T.ink;
  if (p.d2_taso != null) { d2v = p.d2_taso; d2c = 'lähde: ' + ({ sm: 'SM', hh: 'H-H', tk: 'TK' }[p.d2_lahde] || 'tekninen') + (p.d2_kattavuus ? ' · ' + p.d2_kattavuus + ' testiä' : ''); }
  else if (p.tki_viimeisin != null) { d2v = Math.round(p.tki_viimeisin / 20 * 10) / 10; d2c = 'lähde: TKI'; }
  ...
  // §28 (Selkeys 1): kasvumittaus-ohjaus kun PHV puuttuu. Taso + kehityskohde näkyvät silti (harmaa + 🌱) —
  // tämä EI korvaa niitä, se selittää ja OHJAA kasvumittaukseen (adoptio). Ei piilotusta.
  if ((p.phv_tila == null || p.phv_tila === '') && rivit.some(function (r) { return r.dimensio === 'D1'; })) {
    h += '<div style="margin-top:10px;font-size:10.5px;line-height:1.5;color:' + T.amber + ';background:rgba(224,160,64,.08);border:.5px solid rgba(224,160,64,.28);border-radius:8px;padding:8px 10px">'
      + '⚠️ <b>Kasvumittaus puuttuu</b> — fyysiset arviot perustuvat ikäoletukseen. Tee kasvumittaus (Testaus, ~3 min/pelaaja) → kehityskohteet tarkentuvat ja PHV-korjaus huomioi kypsyyden. '
      + '<a href="TalentMaster_Testaus_v9.html" style="color:' + T.teal + ';text-decoration:none;font-weight:600;white-space:nowrap">→ Tee kasvumittaus</a></div>';
  }
  h += ryhma('Fyysinen (D1)', 'D1');
  h += ryhma('Tekniikka (D2)', 'D2');
  if (rivit.length) {
    var onOletus = rivit.some(function (r) { return r.oletus; });
    h += '<div style="margin-top:8px;font-size:10px;color:' + T.ink3 + ';line-height:1.5">🌱 = kypsyys huomioitu (ennen kasvupyrähdystä) — taso viitteellinen, ei kehityskohde · ⭐ = X-Factor (huipputaso)'
      + (onOletus ? ' · <span style="color:' + T.amber + '">ikäoletus</span> = PHV mittaamatta, taso ikänormista — tarkentuu kasvumittauksella' : '')
      + ' · fyysinen 1–5 · lajitekniikka (Eerikkilä H-H) 1–3 · TK-lajitaso (kilpailukohortti) 1–5</div>';
  }
  return h;
}
...
if (typeof module !== 'undefined' && module.exports) {
  ...
  module.exports.taydennaHvSm = taydennaHvSm;
  module.exports.tasoJakauma = tasoJakauma;
  module.exports.tkiTavoiteJakauma = tkiTavoiteJakauma;
  module.exports.tavoiteRadarAkselit = tavoiteRadarAkselit;
  module.exports.radarPatteristotSaatavilla = radarPatteristotSaatavilla;
  module.exports.painopisteOminaisuus = painopisteOminaisuus;
  module.exports.kattavuusVajeet = kattavuusVajeet;
  module.exports.valitseKohortti = valitseKohortti;
}
