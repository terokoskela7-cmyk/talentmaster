...
function laskeVPTuloskortti(pelaajat, valmentajat, tavoitteet, nyt) {
  pelaajat = pelaajat || []; valmentajat = valmentajat || [];
  ...
  var comp = function (p) { var arr = [p.d1_taso, p.hh_taso, p.d2_taso, (p.tki_viimeisin != null ? p.tki_viimeisin / 20 : null)].filter(function (x) { return x != null; }); return arr.length ? Math.max.apply(null, arr) : null; };
  var M = {};
  ...
  // ── II Pelaajakehitys ──
  var onTaso = pelaajat.filter(function (p) { return comp(p) != null; });
  M.taso3_pct = pct(onTaso.filter(function (p) { return comp(p) >= 3; }).length, onTaso.length);
  ...
  var II = { kpi: [
    { nimi: 'Taso ≥3 -osuus', arvo: M.taso3_pct != null ? M.taso3_pct + '%' : '—', status: st(M.taso3_pct, 40, 20), n: onTaso.length },
