/* ══════════════════════════════════════════════════════════════════════════
   ...
   Vaihe 1 (arkkitehtuuri): irrotettu Masterin inline-toteutuksesta sellaisenaan.
   ...
   arvoilla. Yksi lähde → ei driftiä. Vartija tests/reflektio_yksi_lahde.test.js
   pitää huolen ettei recorder-koodi valu takaisin inlineen.

   RIIPPUVUUDET INJEKTOIDAAN (mount) — lib EI lue sovelluksen globaaleja:
   Master ja VP eroavat juuri niissä (masterT vs vpT, eri refresh-funktio),
   joten globaalien lukeminen sitoisi libin Masteriin. Getterit, koska _db/
   _auth/_seuraId vaihtuvat kirjautumisen yhteydessä mount-hetken jälkeen.
...
(function (global) {
  ...
  /* ── Tila + vakiot (siirretty inlinestä sellaisenaan) ──────────────────── */
  function _refFmt(s) { s = Math.round(s || 0); var m = Math.floor(s / 60), x = s % 60; return m + ':' + (x < 10 ? '0' : '') + x; }
  var _REF_MAX_S = 180;                      // 3 min auto-stop
  var _REF_MAX_BYTES = 12 * 1024 * 1024;     // ~12 MB kokokatto
  var _refMime = null, _refExt = null, _refRecorder = null, _refStream = null, _refChunks = [], _refBlob = null, _refSeconds = 0, _refTimer = null, _refTila = 'idle'; // idle|recording|recorded
  var _refTranskriptio = null;   // opt-in Whisper-litterointi (aiProxy voice_transcribe)
  var _refTapahtuma = null;      // valinnainen harjoituskonteksti (Vaihe 2) — null = itsenäinen reflektio

  function _blobToBase64(blob) { return new Promise(function (resolve, reject) { var r = new FileReader(); r.onload = function () { var s = String(r.result || ''); var i = s.indexOf(','); resolve(i >= 0 ? s.slice(i + 1) : s); }; r.onerror = reject; r.readAsDataURL(blob); }); }

  function _refValitseMime() {
  ...
  function _refExtFor(mime) { mime = mime || ''; return mime.indexOf('mp4') >= 0 ? 'mp4' : mime.indexOf('ogg') >= 0 ? 'ogg' : 'webm'; }

  function _refReset() {
    if (_refTimer) { clearInterval(_refTimer); _refTimer = null; }
    if (_refRecorder && _refRecorder.state !== 'inactive') { try { _refRecorder.stop(); } catch (e) {} }
    if (_refStream) { try { _refStream.getTracks().forEach(function (t) { t.stop(); }); } catch (e) {} _refStream = null; }
    _refRecorder = null; _refChunks = []; _refBlob = null; _refSeconds = 0; _refTila = 'idle'; _refTranskriptio = null;
  }

  function _refRenderRec() {
    var btn = document.getElementById('refRecBtn'), aika = document.getElementById('refRecAika'), prev = document.getElementById('refPreview');
    if (!btn) return;
    if (_refTila === 'recording') { btn.textContent = _t('■ Lopeta'); btn.style.background = 'var(--red)'; btn.style.color = '#fff'; btn.style.border = 'none'; if (prev) prev.innerHTML = ''; }
    else if (_refTila === 'recorded') {
      ...
      if (aika) aika.textContent = '🎙 ' + _refFmt(_refSeconds);
      if (prev && _refBlob) {
        var url = URL.createObjectURL(_refBlob);
        var pv = '<audio controls src="' + url + '" style="width:100%;height:36px"></audio>';
        if (_refTranskriptio) pv += '<div style="font-size:12px;color:var(--ink2);background:var(--card);border:.5px solid var(--line);border-radius:8px;padding:8px 10px;margin-top:6px"><b style="color:var(--teal)">' + _t('Litterointi:') + '</b> ' + _esc(_refTranskriptio) + '</div>';
        else pv += '<div style="margin-top:6px;display:flex;align-items:center;gap:8px;flex-wrap:wrap"><button type="button" onclick="_refLitteroi()" id="refLitteroiBtn" style="font-size:11px;padding:5px 12px;border:.5px solid var(--teal);background:rgba(40,176,144,.1);color:var(--teal);border-radius:7px;cursor:pointer">' + _t('✍ Litteroi (valinnainen)') + '</button><span style="font-size:10px;color:var(--ink3)">' + _t('ääni lähetetään litteroitavaksi (opt-in)') + '</span></div>';
        prev.innerHTML = pv;
      }
    } else { btn.textContent = '● Nauhoita'; btn.style.background = 'var(--teal)'; btn.style.color = '#fff'; btn.style.border = 'none'; if (aika) aika.textContent = ''; if (prev) prev.innerHTML = ''; }
  }
  ...
  function refRecToggle() {
    if (_refTila === 'recording') { _refStop(); return Promise.resolve(); }
    if (_refTila === 'recorded') { _refReset(); _refRenderRec(); }
    return navigator.mediaDevices.getUserMedia({ audio: true }).then(function (stream) {
      _refStream = stream;
      _refMime = _refValitseMime(); _refExt = _refExtFor(_refMime);
      try { _refRecorder = _refMime ? new MediaRecorder(_refStream, { mimeType: _refMime }) : new MediaRecorder(_refStream); }
      catch (e) { _refRecorder = new MediaRecorder(_refStream); }
      if (!_refMime) { _refMime = _refRecorder.mimeType || 'audio/webm'; _refExt = _refExtFor(_refMime); }
      _refChunks = []; _refBlob = null; _refSeconds = 0;
      _refRecorder.ondataavailable = function (e) { if (e.data && e.data.size) _refChunks.push(e.data); };
      _refRecorder.onstop = function () {
        _refBlob = new Blob(_refChunks, { type: _refMime });
        if (_refStream) { try { _refStream.getTracks().forEach(function (t) { t.stop(); }); } catch (e) {} _refStream = null; }
        _refTila = 'recorded'; _refRenderRec();
      };
      _refRecorder.start();
      _refTila = 'recording'; _refRenderRec();
      _refTimer = setInterval(function () {
        _refSeconds++;
        var a = document.getElementById('refRecAika');
        if (a) a.textContent = '● ' + _refFmt(_refSeconds) + ' / 3:00';
        if (_refSeconds >= _REF_MAX_S) _refStop();
      }, 1000);
    }, function () { _toast(_t('Mikrofonin käyttö estetty'), 'error'); });
  }

  /* Opt-in Whisper-litterointi (aiProxy voice_transcribe — olemassa oleva CF §13). */
  function refLitteroi() {
    if (!_refBlob) return Promise.resolve();
    var au = _auth(); var cu = au && au.currentUser;
    if (!cu) { _toast(_t('Kirjaudu ensin'), 'error'); return Promise.resolve(); }
    var btn = document.getElementById('refLitteroiBtn'); if (btn) { btn.disabled = true; btn.textContent = _t('Litteroidaan…'); }
    return _blobToBase64(_refBlob).then(function (base64) {
      return cu.getIdToken().then(function (idToken) {
        return fetch('https://europe-west1-' + _projectId() + '.cloudfunctions.net/aiProxy', {
          ...
          body: JSON.stringify({ task: 'voice_transcribe', data: { audio: base64, mimeType: _refMime || 'audio/webm', language: 'fi' } })
        });
      });
    }).then(function (res) {
      ...
      _refTranskriptio = String((j && (j.text || j.transkriptio)) || '').trim();
      if (!_refTranskriptio) throw new Error('tyhjä litterointi');
      _refRenderRec();
  ...
  function avaa(opts) {
    ...
    var audioTuettu = !!_refValitseMime() && !!(navigator.mediaDevices && navigator.mediaDevices.getUserMedia);
    var h = '<div onclick="event.stopPropagation()" style="background:var(--card);border:.5px solid var(--line);border-radius:14px;width:min(540px,96vw);padding:20px 22px;margin:auto">'
      ...
      + (audioTuettu ? '<div id="refRecAlue"><button type="button" id="refRecBtn" onclick="_refRecToggle()" style="padding:8px 16px;border-radius:8px;border:none;background:var(--teal);color:#fff;font-size:12px;cursor:pointer">' + _t('● Nauhoita') + '</button> <span id="refRecAika" style="font-size:12px;color:var(--ink3)"></span></div><div id="refPreview" style="margin-top:8px"></div>' : '<div style="font-size:11px;color:var(--ink3)">' + _t('Äänitallennus ei ole tuettu tässä selaimessa — voit silti kirjoittaa reflektion.') + '</div>')
      + '</div>'
  ...
  function sulje() { _refReset(); _refTapahtuma = null; var m = document.getElementById('refModal'); if (m) m.remove(); }
  ...
  function tallenna() {
    ...
    if (!onTekstia && !_refBlob) { _toast(_t('Kirjoita tai nauhoita reflektio'), 'error'); return Promise.resolve(); }
    if (_refBlob && _refBlob.size > _REF_MAX_BYTES) { _toast(_t('Äänitiedosto liian iso (max ~12 MB)'), 'error'); return Promise.resolve(); }
    ...
    return cu.getIdToken(true).then(function () {   // §7.2
      ...
      if (!_refBlob) return refDoc;
      var path = 'seurat/' + sid + '/kayttajat/' + cu.uid + '/reflektiot/' + refDoc.id + '.' + (_refExt || 'webm');
      var sref = global.firebase.storage().ref(path);
      return sref.put(_refBlob, { contentType: _refMime || 'audio/webm' }).then(function () {
        return sref.getDownloadURL();
      }).then(function (url) {
        doc.audio_url = url;
        doc.audio_path = path;
        doc.audio_kesto_s = _refSeconds;
        if (_refTranskriptio) doc.transkriptio = _refTranskriptio;   // litterointi audion rinnalle (hakukelpoinen)
        return refDoc;
      });
  ...
  function merkintaHTML(rf) {
    ...
    var audioHtml = rf.audio_url ? '<audio controls preload="none" src="' + _esc(rf.audio_url) + '" style="width:100%;margin-top:6px;height:36px"></audio>' + (rf.audio_kesto_s ? '<span style="font-size:10px;color:var(--ink3);margin-left:4px">🎙 ' + _refFmt(rf.audio_kesto_s) + '</span>' : '') + (rf.transkriptio ? '<div style="font-size:12px;color:var(--ink2);margin-top:4px"><span style="color:var(--ink3)">📝</span> ' + _esc(rf.transkriptio) + '</div>' : '') : '';
  ...
  /* HUOM briiffin signatuuriin: mount(seuraId, uid, db) ei riitä — lib tarvitsee
     ...
     Niiden lukeminen globaaleista sitoisi libin Masteriin, mikä on juuri se
     mitä tämä irrotus purkaa. Siksi optiot-objekti. */
  function mount(opts) {
    cfg = opts || {};
    ...
    global._refRecToggle = refRecToggle;
    global._refLitteroi = refLitteroi;
  ...
  var API = {
    ...
    fmtKesto: _refFmt,
    MAX_S: _REF_MAX_S,
    MAX_BYTES: _REF_MAX_BYTES,
    audioTuettu: function () { return !!_refValitseMime() && !!(navigator.mediaDevices && navigator.mediaDevices.getUserMedia); }
  };
