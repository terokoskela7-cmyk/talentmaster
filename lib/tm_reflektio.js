...
(function (global) {
  ...
  function tallenna() {
    ...
    return cu.getIdToken(true).then(function () {   // §7.2
      ...
      /* AKTIIVISUUSLASKURI: VP ei voi lukea yksityistä reflektiopäiväkirjaa
         (storage/firestore-sääntö sallii vain oman uid:n), joten aktiivisuus
         välittyy LUKUMÄÄRÄNÄ valmentajan omaan kayttajat-dokkiin. VP näkee
         luvun, ei sisältöä. Best-effort — ei saa kaataa tallennusta. */
      try {
        var laskuriData = {};
        laskuriData.reflektiot_n = global.firebase.firestore.FieldValue.increment(1);
        laskuriData.reflektiot_viim = global.firebase.firestore.FieldValue.serverTimestamp();
        _db().collection('seurat').doc(sid).collection('kayttajat').doc(cu.uid)
          .update(laskuriData)['catch'](function () {});
      } catch (e) {}
  ...
  function merkintaHTML(rf) {
    ...
    var audioHtml = global.tmAani.toistoHTML(rf);   // jaettu toistorenderöijä (sama kaikilla pinnoilla)
