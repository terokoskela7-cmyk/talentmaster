...
(function (root) {
  ...
  function tmHhSnapshot(pvm, src) {
    ...
    // K1b §22 — alusta per piste (nopeus/ketteryys vertailukelpoisia vain saman alustan sisällä). null/tyhjä → ei merkitä
    // (vertailukelpoinen, ei regressiota). Render-vartija (tm_kehityskaari) segmentoi vasta kun eksplisiittiset alustat eroavat.
    if (src.alusta != null && String(src.alusta).trim() !== '') snap.alusta = String(src.alusta).trim();
    return snap;
  }
