...
function _keUusiId(spec, etuliite) {
  ...
  ((spec.selitteet) || []).forEach(function (x) { kaytossa[x.id] = 1; });
  ((spec.korkeuslinjat) || []).forEach(function (x) { kaytossa[x.id] = 1; });
  ((spec.peittovarjot) || []).forEach(function (x) { kaytossa[x.id] = 1; });
...
function kaavioOsuma(spec, x, y, kynnys) {
  ...
  var janaEtaisyys = function (a, b) {
    var dx = b.x - a.x, dy = b.y - a.y, pit2 = dx * dx + dy * dy;
    var t = pit2 ? Math.max(0, Math.min(1, ((x - a.x) * dx + (y - a.y) * dy) / pit2)) : 0;
    return etaisyys(a.x + t * dx, a.y + t * dy);
  };
  pd = raja;
  (s.liikkeet || []).forEach(function (l) {
    var a = piste(l.from), b = piste(l.to);
    if (!a || !b) return;
    var d = janaEtaisyys(a, b);
    if (d <= pd) { pd = d; osui = { tyyppi: 'liike', id: l.id }; }
  });
  if (osui) return osui;
  /* LUOKKA C. Järjestys on tarkoituksellinen: tarkat elementit ensin, VYÖHYKE VIIMEISENÄ.
     Vyöhyke on iso suorakulmio — jos se testattaisiin ensin, se söisi kaikki klikkaukset
     alueeltaan eikä sisällä olevia pelaajia voisi enää valita. */
  pd = raja;
  (s.korkeuslinjat || []).forEach(function (kl) {
    var d = Math.abs(kl.y - y);
    if (d <= pd) { pd = d; osui = { tyyppi: 'korkeuslinja', id: kl.id }; }
  });
  if (osui) return osui;
  pd = raja;
  (s.peittovarjot || []).forEach(function (pv) {
    var a = piste({ ref: pv.from }), b = piste({ ref: pv.to });
    if (!a || !b) return;
    var d = janaEtaisyys(a, b);
    if (d <= pd) { pd = d; osui = { tyyppi: 'peittovarjo', id: pv.id }; }
  });
  if (osui) return osui;
  if (s.vyohyke) {
    var z = s.vyohyke;
    // Kulma ensin: sen osuma-alue on suorakulmion sisällä, joten rungon testi veisi sen.
    if (etaisyys(z.x + z.w, z.y + z.h) <= raja) return { tyyppi: 'vyohyke', id: 'vyohyke', kulma: true };
    if (x >= z.x && x <= z.x + z.w && y >= z.y && y <= z.y + z.h) return { tyyppi: 'vyohyke', id: 'vyohyke' };
  }
  return osui;
}
...
function kaavioPoista(spec, id) {
  ...
  // Peittovarjo poistuu KAHDESTA syystä: joko se itse valittiin (pv.id === id) tai sen pää
  // poistettiin (kaskadi). Ilman jälkimmäistä validaattori hylkäisi tallennuksen orvon viitteen
  // takia — virheellä jota käyttäjä ei osaa yhdistää pelaajan poistoon.
  s.peittovarjot = (s.peittovarjot || []).filter(function (pv) { return pv.id !== id && pv.from !== id && pv.to !== id; });
  if (!s.peittovarjot.length) delete s.peittovarjot;
  s.korkeuslinjat = (s.korkeuslinjat || []).filter(function (kl) { return kl.id !== id; });
  if (!s.korkeuslinjat.length) delete s.korkeuslinjat;
  if (id === 'vyohyke') delete s.vyohyke;   // vyöhykkeellä ei ole omaa id:tä — yksi per kaavio
...
/* ══════════ LUOKKA C: PEITTOVARJO · KORKEUSLINJA · VYÖHYKE ═══════════════════════════════
   Renderöijä piirsi nämä jo, mutta editorissa ei ollut keinoa tehdä omia — valmentaja näki
   kanonisessa kirjastossa elementtejä joita ei voinut itse piirtää. Muoto on LUKITTU
   renderöijää vasten (tm_kaavio_render.js): vyohyke{x,y,w,h} · korkeuslinjat[{id,y}] ·
   peittovarjot[{id,from,to}].

   Kaikki funktiot ovat PUHTAITA (kopio sisään, kopio ulos) ja klamppaavat validaattorin
   rajoihin — käyttäjä ei saa raahaamalla tuottaa speciä jonka tallennus hylkää. */

/* Peittovarjo = syöttölinja jonka puolustaja peittää kehollaan. from = peittäjä, to = kohde.
   Oletukset: ensimmäinen OMA (peittäjä) ja ensimmäinen VASTUSTAJA — tavallisin puolustusasetelma.
   Jos jompaakumpaa ei ole, käytetään kahta ensimmäistä pelaajaa; alle kahdella ei voi peittää. */
function kaavioLisaaPeittovarjo(spec, fromId, toId) {
  var s = _keKopio(spec), pl = s.pelaajat || [];
  if (pl.length < 2) return { spec: s, virhe: 'Peittovarjo vaatii kaksi pelaajaa' };
  var oma = pl.filter(function (p) { return p.joukkue === 'oma'; })[0];
  var vast = pl.filter(function (p) { return p.joukkue === 'vastustaja'; })[0];
  var f = fromId || (oma ? oma.id : pl[0].id);
  var t = toId || (vast ? vast.id : pl.filter(function (p) { return p.id !== f; })[0].id);
  var on = function (id) { return pl.some(function (p) { return p.id === id; }); };
  if (!on(f) || !on(t)) return { spec: s, virhe: 'Peittovarjo: tuntematon pelaaja' };
  if (f === t) return { spec: s, virhe: 'Peittovarjo: päät eivät voi olla sama pelaaja' };
  var pv = { id: _keUusiId(s, 'pv'), from: f, to: t };
  s.peittovarjot = (s.peittovarjot || []).concat([pv]);
  return { spec: s, id: pv.id };
}
/* Pään vaihto paneelista. from===to torjutaan TÄSSÄ eikä vasta tallennuksessa: käyttäjä saisi
   muuten virheilmoituksen vasta minuutteja myöhemmin, irrallaan teostaan. */
function kaavioAsetaPeittovarjoPaa(spec, id, paa, pelaajaId) {
  var s = _keKopio(spec);
  if (paa !== 'from' && paa !== 'to') return { spec: s, virhe: 'Tuntematon pää' };
  if (!(s.pelaajat || []).some(function (p) { return p.id === pelaajaId; })) return { spec: s, virhe: 'Tuntematon pelaaja' };
  var pv = (s.peittovarjot || []).filter(function (x) { return x.id === id; })[0];
  if (!pv) return { spec: s, virhe: 'Peittovarjoa ei löydy' };
  var toinen = (paa === 'from') ? pv.to : pv.from;
  if (pelaajaId === toinen) return { spec: s, virhe: 'Peittovarjo: päät eivät voi olla sama pelaaja' };
  pv[paa] = pelaajaId;
  return { spec: s, id: id };
}

/* Korkeuslinja = puolustuslinjan korkeus / prässin laukaisulinja. Vain y — linja on aina
   kentän levyinen (renderöijä piirtää x 2..98). */
function kaavioLisaaKorkeuslinja(spec, y) {
  var s = _keKopio(spec);
  var kl = { id: _keUusiId(s, 'kl'), y: _keKlamppi(y == null ? 50 : y, 0, 100) };
  s.korkeuslinjat = (s.korkeuslinjat || []).concat([kl]);
  return { spec: s, id: kl.id };
}
function kaavioSiirraKorkeuslinja(spec, id, y) {
  var s = _keKopio(spec);
  var kl = (s.korkeuslinjat || []).filter(function (x) { return x.id === id; })[0];
  if (!kl) return { spec: s, virhe: 'Korkeuslinjaa ei löydy' };
  kl.y = _keKlamppi(y, 0, 100);
  return { spec: s, id: id };
}

/* Vyöhyke = korostettu alue, YKSI per kaavio (objekti, ei taulukko — renderöijän muoto).
   `false` poistaa. Klamppaus pitää suorakulmion kentällä MYÖS silloin kun x+w ylittäisi 100:
   leveyttä kavennetaan ennemmin kuin siirretään laatikkoa, jotta raahattu kulma pysyy siellä
   minne käyttäjä sen vei. */
var _KE_VYOHYKE_MIN = 4;
function kaavioAsetaVyohyke(spec, asetukset) {
  var s = _keKopio(spec);
  if (asetukset === false) { delete s.vyohyke; return { spec: s }; }
  var v = s.vyohyke || { x: 30, y: 35, w: 40, h: 30 };
  var a = asetukset || {};
  var x = _keKlamppi(a.x == null ? v.x : a.x, 0, 100 - _KE_VYOHYKE_MIN);
  var y = _keKlamppi(a.y == null ? v.y : a.y, 0, 100 - _KE_VYOHYKE_MIN);
  var w = _keKlamppi(a.w == null ? v.w : a.w, _KE_VYOHYKE_MIN, 100 - x);
  var h = _keKlamppi(a.h == null ? v.h : a.h, _KE_VYOHYKE_MIN, 100 - y);
  s.vyohyke = { x: x, y: y, w: w, h: h };
  return { spec: s };
}
...
var TM_KAAVIO_EDITORI = {
  ...
  kaavioLisaaPeittovarjo: kaavioLisaaPeittovarjo, kaavioAsetaPeittovarjoPaa: kaavioAsetaPeittovarjoPaa,
  kaavioLisaaKorkeuslinja: kaavioLisaaKorkeuslinja, kaavioSiirraKorkeuslinja: kaavioSiirraKorkeuslinja,
  kaavioAsetaVyohyke: kaavioAsetaVyohyke
};
