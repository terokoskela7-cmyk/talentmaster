...
(function (global) {
  ...
  /**
   * TALLENNETUN äänen toisto: soitin + kesto + litterointi.
   *
   * Yksi renderöijä kolmelle pinnalle (reflektiopäiväkirja · VP:n palautelista ·
   * valmentajan "Saatu palaute"). Sama ääni näyttää samalta riippumatta siitä
   * kuka sen kuulee — ja litterointi ei voi jäädä näyttämättä yhdellä pinnalla
   * siksi että sen renderi unohtui kopioida sinne.
   *
   * @param {{audio_url,audio_kesto_s,transkriptio}} o  tyhjä merkkijono jos ei ääntä
   */
  function toistoHTML(o) {
    o = o || {};
    if (!o.audio_url) return '';
    return '<audio controls preload="none" src="' + _esc(o.audio_url) + '" style="width:100%;margin-top:6px;height:36px"></audio>'
      + (o.audio_kesto_s ? '<span style="font-size:10px;color:var(--ink3);margin-left:4px">🎙 ' + fmtKesto(o.audio_kesto_s) + '</span>' : '')
      + (o.transkriptio ? '<div style="font-size:12px;color:var(--ink2);margin-top:4px"><span style="color:var(--ink3)">📝</span> ' + _esc(o.transkriptio) + '</div>' : '');
  }
