...
/* NÄKYVYYSTASOT. Kolme ensimmäistä ovat PELAAJAYLEISÖJÄ: ne vastaavat kysymykseen "kuka pelaaja
   näkee tämän kuvan". 'valmentaja' on eri akseli — HENKILÖSTÖREITITYS: kenelle valmentajalle kuva
   annetaan käytettäväksi. Siksi se EI ole pelaajayleisö (kaavioKohdistuu → false); valmentaja ottaa
   sen käyttöön ja kohdistaa halutessaan pelaajille erikseen omana kaavionaan. */
var TM_KAAVIO_NAKYVYYDET = ['seura', 'joukkue', 'valmentaja', 'pelaaja'];

/* Mikä taso vaatii KOHTEEN (id) pelkän tason lisäksi. 'seura' ei vaadi — kohde on koko seura.
   Tämä on se puuttunut puolisko: aiemmin UI tallensi tason mutta jätti kohteen tyhjäksi, jolloin
   kortti sanoi "pelaaja" muttei kenelle — eikä VP voinut tietää kenelle kaavio oli tarkoitettu. */
var TM_KAAVIO_KOHDEKENTTA = { joukkue: 'joukkueId', valmentaja: 'valmentajaId', pelaaja: 'pelaajaIds' };
function kaavioKohdeKentta(nakyvyys) { return TM_KAAVIO_KOHDEKENTTA[nakyvyys] || null; }
/* Puuttuuko kohde vaikka taso vaatii sen. UI VAROITTAA tästä muttei estä: VP saa tarkentaa
   kohdistuksen katselmuksessa, ja esto tekisi keskeneräisen luonnoksen tallentamisesta mahdotonta. */
function kaavioKohdePuuttuu(doc) {
  var r = (doc && doc.review) || {};
  var k = kaavioKohdeKentta(r.nakyvyys);
  if (!k) return false;
  var v = r[k];
  return Array.isArray(v) ? v.length === 0 : !v;
}

/* Mitkä tasot TÄMÄ tekijä saa asettaa. Peili firestore.rulesin kaavioNakyvyysLuontiOk():sta —
   UI ei saa kovakoodata rooleja, ja tämän kanssa valikko ja sääntö eivät voi ajautua erilleen. */
function kaavioNakyvyysTasot(ctx) {
  var hyv = kaavioOnHyvaksyja(ctx);
  return hyv ? ['seura', 'joukkue', 'valmentaja', 'pelaaja'] : ['joukkue', 'valmentaja', 'pelaaja'];
}
...
/* YKSITYINEN LUONNOS (erä D2). Valmentaja haluaa hioa piirroksen rauhassa ennen kuin se on
   esillä. Tämä on "ei vielä valmis", EI valmentajien välinen piilotus — seuran sisäinen
   näkyvyys on tarkoituksellista, ja se palaa heti julkaisusta.

   ⚠ PÄÄTÖS JOTA EI OLE VAHVISTETTU: näkeekö johto (VP/UTJ) keskeneräiset? Toteutettu tiukempi
   linja — EI näe, koska "yksityinen" tarkoittaa ei valmista KENELLEKÄÄN. SA näkee (ylläpito).
   Jos johto tarvitsee oversightin keskeneräisiin, muutos on tämän yhden funktion ehto +
   rulesin kaavioLukuOk() — molemmat merkitty samalla kommentilla.

   Taaksepäin: puuttuva kenttä = julkinen. Vanhat luonnokset eivät katoa kenenkään näkyvistä. */
function kaavioYksityinen(doc) {
  var r = (doc && doc.review) || {};
  return r.yksityinen === true && r.status !== 'hyvaksytty';
}
function kaavioOnTekija(doc, ctx) {
  var c = _kaavioCtx(ctx), r = (doc && doc.review) || {};
  return !!c.uid && r.luonut === c.uid;
}
...
function kaavioVoiLukea(doc, ctx) {
  ...
  if (c.superAdmin) return true;
  // Yksityinen luonnos: vain tekijä. Ehto on ENNEN seura-haaraa, koska se kaventaa senkin.
  if (kaavioYksityinen(doc) && !kaavioOnTekija(doc, c)) return false;
...
function kaavioKohdistuu(doc, pelaaja) {
  ...
  // HENKILÖSTÖREITITYS, ei pelaajayleisö: valmentajalle osoitettu kaavio ei valu pelaajalle.
  // Tämä oli jo tosi tuntemattoman tason fail-closed-haaran kautta; eksplisiittinen rivi tekee
  // siitä luettavan ja testattavan sen sijaan että se riippuisi loppufallbackista.
...
function kaavioToiminnot(doc, ctx) {
  ...
  if (kaavioVoiKirjoittaa(doc, c)) {
    A.push('muokkaa');
    // Julkaisu on TEKIJÄN toimi: hän päättää milloin työ on esittelykelpoista. Muille se ei ole
    // tarjolla — eivät he sitä näekään (kaavioVoiLukea), mutta SA näkee kaiken eikä saa julkaista
    // toisen keskeneräistä.
    if (kaavioYksityinen(doc) && kaavioOnTekija(doc, c)) A.push('julkaise');
...
var TM_KAAVIO_POLICY = {
  ...
  TM_KAAVIO_KOHDEKENTTA: TM_KAAVIO_KOHDEKENTTA, kaavioKohdeKentta: kaavioKohdeKentta,
  kaavioKohdePuuttuu: kaavioKohdePuuttuu, kaavioNakyvyysTasot: kaavioNakyvyysTasot,
  ...
  kaavioYksityinen: kaavioYksityinen, kaavioOnTekija: kaavioOnTekija,
