...
// P1 (§13 LUKITTU): ADAR 1–5. Vanha 1–10-data normalisoidaan lukuvaiheessa (>5 → ÷2). 1–5/1–3 = suoraan (pyöristä+cap).
function tmAdarHavaittu(adarViimeisin) {
  if (!adarViimeisin) return null;
  var norm = function (v) { if (v == null) return null; var x = (v > 5) ? Math.round(v / 2) : Math.round(v); return Math.max(1, Math.min(5, x)); };
