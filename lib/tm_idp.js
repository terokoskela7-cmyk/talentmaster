...
/* ── TAVOITETAULUKKO ON HISTORIA ───────────────────────────────────────────────────────────────
   HOTFIX 23.9.: tallennus kirjoitti `{ tavoitteet: [t] }` merge:llä. Firestoren merge EI yhdistä
   taulukoita vaan korvaa kentän kokonaan → jokainen tallennus jätti taulukkoon yhden tavoitteen ja
   PYYHKI vanhan tavoitteen sekä sen `arviot`-kehityskeskustelut pysyvästi. Lukijat olettivat jo
   historiaa (arr[arr.length-1]); vain tallennus rikkoi sen.

   idpTavoitteetYhdista(vanhat, t, opts) → UUSI taulukko. EI mutatoi syötettä.
     · tunniste = t.luotu (idpRakennaTavoite antaa sen jokaiselle tavoitteelle)
     · luotu löytyy  → korvaa VAIN se alkio (kattaa muokkauksen, kehityskeskustelun ja elinkaaren)
     · uusi luotu    → lisätään loppuun; voimassa oleva (aktiivinen|jatkuu|saavutettu) merkitään
                       status:'vaihdettu' + vaihdettu_pvm. Sen arviot ja muut kentät SÄILYVÄT.
     · ehdotettu     → ei vaihda mitään; vasta hyväksyntä (sama luotu, status aktiivinen) vaihtaa.
   opts: { korvaa:false → älä merkitse vanhaa vaihdetuksi · nyt: ISO|Date|ms (determinismi) } */
var IDP_VOIMASSA = ['aktiivinen', 'jatkuu', 'saavutettu'];
function idpOnVoimassa(t) { return !!t && IDP_VOIMASSA.indexOf(t.status) >= 0; }
/* Voimassa oleva = viimeisin jonka status EI ole 'vaihdettu' eikä 'hylatty'. Lukijoiden AINOA valitsin:
   taulukon viimeinen alkio sokeasti voisi olla juuri vaihdettu tai hylätty. */
function idpVoimassaTavoite(tavoitteet) {
  var a = Array.isArray(tavoitteet) ? tavoitteet : [];
  for (var i = a.length - 1; i >= 0; i--) {
    var t = a[i];
    if (t && t.status !== 'vaihdettu' && t.status !== 'hylatty') return t;
  }
  return null;
}
/* Vuodenvaihde: doc-id on kalenterivuosi, joten 1.1. syksyn tavoite katoaisi näkyvistä ja seuraava
   tallennus aloittaisi tyhjästä dokista. Valinta: kuluva vuosi ensin, muuten edellinen.
   Palauttaa { tavoite, vuosi } tai null. Ei doc-ID-mallin muutosta, ei migraatiota. */
function idpValitseKausidokista(nytArr, nytVuosi, edellArr, edellVuosi) {
  /* VOIMASSA OLEVA (IDP_VOIMASSA), EI ehdotus: idpVoimassaTavoite laskee 'ehdotettu'-tilan mukaan, joten
     tammikuussa kuluvan vuoden pelkka ehdotus olisi nayttanyt riittavalta ja edellisen vuoden AKTIIVINEN
     tavoite olisi jaanyt lukematta (ja merkitsematta vaihdetuksi hyvaksynnassa). idpVoimassaTavoite jaa
     ennalleen muille kutsujille. */
  var t = idpJaaVoimassaJaEhdotus(nytArr).tavoite;
  if (t) return { tavoite: t, vuosi: nytVuosi };
  var e = idpJaaVoimassaJaEhdotus(edellArr).tavoite;
  if (e) return { tavoite: e, vuosi: edellVuosi };
  return null;
}
/* Lataajien yhteinen valinta KAHDESTA vuosidokista (vuodenvaihde). Voimassa oleva tavoite ja odottava
   ehdotus voivat olla ERI dokeissa: tammikuussa [A aktiivinen] on 2026-dokissa ja valmentajan ehdotus C
   2027-dokissa. Palauttaa molemmat lahdevuosineen, jotta tallennus osuu oikeaan dokkiin ja vanha tavoite
   merkitaan vaihdetuksi siella missa se on.
   { tavoite, tavoiteVuosi, luonnos, luonnosVuosi } — kukin voi olla null. */
function idpValitseKahdestaVuodesta(nytArr, nytVuosi, edellArr, edellVuosi) {
  var val = idpValitseKausidokista(nytArr, nytVuosi, edellArr, edellVuosi);
  var nyt = idpJaaVoimassaJaEhdotus(nytArr);
  var out = {
    tavoite: val ? val.tavoite : null,
    tavoiteVuosi: val ? val.vuosi : null,
    luonnos: nyt.luonnos,
    luonnosVuosi: nyt.luonnos ? nytVuosi : null
  };
  /* Edellisen vuoden ehdotus otetaan vain jos VOIMASSA OLEVA tuli sielta (sama dokki) — muuten vanha,
     syrjaytynyt ehdotus nousisi luonnokseksi kuluvan vuoden tavoitteen rinnalle. Kuluvan vuoden ehdotus
     voittaa aina (uudempi). */
  if (!out.luonnos && val && val.vuosi === edellVuosi) {
    var ed = idpJaaVoimassaJaEhdotus(edellArr);
    if (ed.luonnos) { out.luonnos = ed.luonnos; out.luonnosVuosi = edellVuosi; }
  }
  return out;
}
/* Vaihe 1 (Kehitys-tyopoyta): voimassa oleva tavoite ja tallennettu ehdotus ERIKSEEN.
   idpVoimassaTavoite palauttaa viimeisimman ei-vaihdetun/ei-hylatyn -- myos 'ehdotettu'-alkion, joten
   [A aktiivinen, B ehdotettu] antaisi B:n voimassa olevaksi. Tyopoyta tarvitsee jaon: rivit, nauha ja
   pikakentat lukevat A:n, B latautuu vain luonnokseksi. { tavoite, luonnos } (kumpikin voi olla null). */
function idpJaaVoimassaJaEhdotus(tavoitteet) {
  var a = Array.isArray(tavoitteet) ? tavoitteet : [];
  var tavoite = null, tavIdx = -1;
  for (var i = a.length - 1; i >= 0; i--) { if (idpOnVoimassa(a[i])) { tavoite = a[i]; tavIdx = i; break; } }
  var luonnos = null;
  for (var j = a.length - 1; j > tavIdx; j--) { if (a[j] && a[j].status === 'ehdotettu') { luonnos = a[j]; break; } }
  return { tavoite: tavoite, luonnos: luonnos };
}
/* Mihin dokkiin tallennetaan: samaan josta tavoite ladattiin (kunnes uusi tavoite luodaan),
   UUSI tavoite aina kuluvan kauden dokkiin. p._idpVuosi / p._idpVuosiLuotu asetetaan latauksessa.
   A3: LADATTU EHDOTUS tallentuu samaan dokkiin josta se luettiin (p._idpLuonnosVuosi/_idpLuonnosLuotu).
   Muuten vuodenvaihteessa hyvaksynta kirjoittaisi sen kuluvan vuoden dokkiin, ja sama-luotu-haara ei
   loytaisi voimassa olevaa tavoitetta merkittavakseen -> kaksi aktiivista tavoitetta eri dokeissa. */
function idpTallennusVuosi(p, t, nyt) {
  var nykyinen = idpKausivuosi(nyt);
  if (p && p._idpLuonnosVuosi && t && t.luotu && p._idpLuonnosLuotu === t.luotu) return p._idpLuonnosVuosi;
  if (p && p._idpVuosi && t && t.luotu && p._idpVuosiLuotu === t.luotu) return p._idpVuosi;
  return nykyinen;
}
/* A3 — vuodenvaihde: voimassa oleva tavoite voi olla TOISESSA vuosidokissa kuin mihin uusi tavoite
   kirjoitetaan (lataus lukee edellisen vuoden dokin fallbackina). Silloin se on merkittava vaihdetuksi
   siella, muuten historiassa on kaksi aktiivista tavoitetta. Puhdas: EI mutatoi syotetta.
   Palauttaa uuden taulukon, tai null kun merkittavaa ei ole (-> kutsujan EI pida kirjoittaa). */
function idpMerkitseVaihdetuksi(tavoitteet, luotu, nyt) {
  var a = Array.isArray(tavoitteet) ? tavoitteet : [];
  if (!luotu) return null;
  var nytIso = nyt ? new Date(nyt).toISOString() : new Date().toISOString();
  var lista = a.slice(), muuttui = false;
  for (var i = 0; i < lista.length; i++) {
    var v = lista[i];
    if (!v || v.luotu !== luotu || !idpOnVoimassa(v)) continue;
    var kopio = {};
    for (var k in v) { if (Object.prototype.hasOwnProperty.call(v, k)) kopio[k] = v[k]; }
    kopio.status = 'vaihdettu';
    kopio.vaihdettu_pvm = nytIso;
    lista[i] = kopio;
    muuttui = true;
  }
  return muuttui ? lista : null;
}
function idpTavoitteetYhdista(vanhat, t, opts) {
  opts = opts || {};
  var lista = Array.isArray(vanhat) ? vanhat.slice() : [];
  if (!t) return lista;
  var nytIso = opts.nyt ? new Date(opts.nyt).toISOString() : new Date().toISOString();
  // Merkitse muut voimassa olevat vaihdetuiksi. ohitaIdx = alkio jota ollaan juuri korvaamassa.
  var merkitseVaihdetut = function (ohitaIdx) {
    for (var k = 0; k < lista.length; k++) {
      if (k === ohitaIdx || !idpOnVoimassa(lista[k])) continue;
      var kopio = {}, avain;
      for (avain in lista[k]) { if (Object.prototype.hasOwnProperty.call(lista[k], avain)) kopio[avain] = lista[k][avain]; }
      kopio.status = 'vaihdettu';
      kopio.vaihdettu_pvm = nytIso;
      lista[k] = kopio;   // arviot ja muut kentät säilyvät
    }
  };
  var tunniste = t.luotu || null;
  if (tunniste) {
    for (var i = 0; i < lista.length; i++) {
      if (lista[i] && lista[i].luotu === tunniste) {
        /* KAKSIVAIHEINEN HYVÄKSYNTÄ: ehdotus tallennettiin ensin omana alkionaan, ja nyt sama `luotu`
           palaa statuksella 'aktiivinen'. Ilman tätä haaraa taulukkoon jäi KAKSI voimassa olevaa
           tavoitetta (ja jos uusi myöhemmin hylättiin, vanha heräsi henkiin). Merkitään muut voimassa
           olevat vaihdetuiksi VAIN kun korvattava alkio EI ollut voimassa ja uusi ON:
             · aktiivinen → saavutettu/jatkuu (voimassa → voimassa) ei muuta mitään
             · ehdotettu → hylatty (uusi ei ole voimassa) ei muuta mitään */
        if (opts.korvaa !== false && !idpOnVoimassa(lista[i]) && idpOnVoimassa(t)) merkitseVaihdetut(i);
        lista[i] = t;
        return lista;
      }
    }
  }
  /* Uusi tavoite. Vain VOIMASSA oleva uusi syrjäyttää edellisen — sama ehto kuin sama-luotu-haarassa.
     (Aiemmin ehto oli `status !== 'ehdotettu'`, jolloin esim. uusi 'hylatty' olisi merkinnyt voimassa
     olevan vaihdetuksi. Luonnos-/Peruuta-polut eivät saa koskaan pudottaa voimassa olevaa.) */
  if (opts.korvaa !== false && idpOnVoimassa(t)) merkitseVaihdetut(-1);
  lista.push(t);
  return lista;
}
...
function idpJumissa(tavoite, now) {
  ...
  // 'vaihdettu' = korvattu historiamerkintä → ei voi olla 'jumissa' (HOTFIX 23.9.).
  if (tavoite.status === 'saavutettu' || tavoite.status === 'hylatty' || tavoite.status === 'vaihdettu') return false;
...
function idpPelaajaKaari(tavoite, opts) {
  ...
  if (status === 'hylatty' || status === 'vaihdettu') return null;   // hylätty/korvattu tavoite ei näy pelaajalle
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = {
    ...
    idpTavoitteetYhdista: idpTavoitteetYhdista,
    idpValitseKausidokista: idpValitseKausidokista,
    idpJaaVoimassaJaEhdotus: idpJaaVoimassaJaEhdotus,
    idpMerkitseVaihdetuksi: idpMerkitseVaihdetuksi,
    idpValitseKahdestaVuodesta: idpValitseKahdestaVuodesta,
    idpTallennusVuosi: idpTallennusVuosi,
    idpVoimassaTavoite: idpVoimassaTavoite,
    idpOnVoimassa: idpOnVoimassa,
