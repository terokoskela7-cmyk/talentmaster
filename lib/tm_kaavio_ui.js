...
/* Odottava havaintoliitos: havaintonäkymä asettaa tämän ENNEN editorin avausta
   (uusi kaavio TAI kaaviopankista valittu). Kiinnitetään muokkausobjektiin
   editorin avautuessa, jotta molemmat polut toimivat ilman omaa editoria. */
var _kaavioOdottavaLiitos = null;
function tmKaavioLiitaHavaintoon(ctx) { _kaavioOdottavaLiitos = ctx || null; }
window.tmKaavioLiitaHavaintoon = tmKaavioLiitaHavaintoon;

function _kaavioAvaaEditori(k) {
  if (_kaavioOdottavaLiitos && k) { k._liitaHavaintoon = _kaavioOdottavaLiitos; _kaavioOdottavaLiitos = null; }
  ...
  if (typeof xtPiirraKerros === 'function') h += '<div id="_kvXt" style="border-top:.5px solid var(--border);padding-top:10px;margin-top:10px">' + _kaavioXtHTML() + '</div>';
...
/* UHKA-ARVO-PANEELI editorissa. Kolme tilaa: pois · arvot (vain liikkeiden lukuarvot) · kartta
   (uhkakartta + arvot). Tila on EDITORIN näkymätila (_kaavioTila.xt), ei speksin kenttä. */
function _kaavioXtHTML() {
  var tila = _kaavioTila.xt || null;
  var nappi = function (arvo, lbl) {
    var on = tila === arvo;
    return '<button onclick="_kaavioXtAseta(' + (arvo ? '\'' + arvo + '\'' : 'null') + ')" style="background:' + (on ? 'rgba(40,176,144,.16)' : 'var(--ov-1)') + ';color:' + (on ? 'var(--teal)' : 'var(--ink2)') + ';border:.5px solid ' + (on ? 'rgba(40,176,144,.45)' : 'var(--border)') + ';border-radius:6px;padding:5px 9px;font-size:11px;cursor:pointer">' + _kuiT(lbl) + '</button>';
  };
  var h = '<div style="font-size:10px;letter-spacing:.1em;text-transform:uppercase;color:var(--ink3);margin-bottom:6px">' + _kuiT('Uhka-arvo (xT)') + '</div>';
  h += '<div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:6px">' + nappi(null, 'Pois') + nappi('arvot', 'Liikkeiden arvot') + nappi('kartta', 'Uhkakartta') + nappi('riski', 'Riskikartta') + '</div>';
  var a = _kaavioTila.xtAnalyysi;
  if (tila && a) {
    var summa = a.yhteensa.pallotapahtumia
      ? _kuiT('Oma joukkue etenee') + ' <b style="color:var(--teal)">' + xtMuotoile(a.yhteensa.pisteet, true) + '</b> ' + _kuiT('uhkapistettä') + ' (' + a.yhteensa.pallotapahtumia + ' ' + _kuiT('syöttöä/kuljetusta') + ')'
      : _kuiT('Lisää syöttö tai kuljetus nähdäksesi uhka-arvon.');
    h += '<div style="font-size:11.5px;color:var(--ink2);line-height:1.45">' + summa + '</div>';
    if (a.juoksuPotentiaali.juoksuja) h += '<div style="font-size:11px;color:var(--ink3);line-height:1.45">' + _kuiT('Juoksut avaavat') + ' ' + xtMuotoile(a.juoksuPotentiaali.pisteet) + ' ' + _kuiT('uhkapistettä, jos pallo tulee perille.') + '</div>';
    if (tila === 'riski') h += '<div style="font-size:11px;color:var(--ink3);line-height:1.45;margin-top:4px">' + _kuiT('Punainen: pallonmenetys tässä antaa vastustajalle suurimman uhan. Keltainen: oma puolustuskolmannes. Rakentelussa tämä on paikka varmalle ratkaisulle.') + '</div>';
    if (a.huomio) h += '<div style="font-size:11px;color:var(--amber);line-height:1.45;margin-top:4px">' + _kuiEsc(_kuiT(a.huomio)) + '</div>';
  }
  return h;
}
function _kaavioXtAseta(tila) { _kaavioTila.xt = tila || null; _kaavioPiirraEditori(); }
window._kaavioXtAseta = _kaavioXtAseta;
...
function _kaavioPiirraEditori() {
  ...
  // UHKA-ARVO (xT) — valinnainen näkymäkerros (lib/tm_xt.js + tm_xt_kerros.js). Sama kuri kuin
  // valintakorostuksessa: piirretään drawSpecin JÄLKEEN, ei kirjoiteta speciin. pointer-events:none
  // → hit-testaus ja veto toimivat ennallaan. Guard: appi joka ei lataa xT-libejä ei näe mitään.
  _kaavioTila.xtAnalyysi = null;
  if (_kaavioTila.xt && typeof xtPiirraKerros === 'function') {
    _kaavioTila.xtAnalyysi = xtPiirraKerros(svg, _kaavioTila.muokkaus.spec, { uhkakartta: _kaavioTila.xt === 'kartta', riski: _kaavioTila.xt === 'riski', arvot: true });
  }
  var _xtEl = document.getElementById('_kvXt');
  if (_xtEl) _xtEl.innerHTML = _kaavioXtHTML();
...
/* ══════════════════════════════════════════════════════════
   ...
   LUOKKA C (peittovarjo · korkeuslinja · vyöhyke) ON NYT MUKANA. Se oli aiemmin jätetty pois
   koska validaattori ei tuntenut kenttiä — työkalu olisi tuottanut dataa jota mikään portti ei
   valvo. Aukko oli mitattava: kentän ulkopuolinen vyöhyke ja olemattomaan pelaajaan osoittava
   peittovarjo menivät molemmat läpi puhtaana. Nyt §6-validaattorissa on portit kaikille kolmelle
   (tm_kaavio_validate.js), joten este on poistunut ja työkalut voivat tulla.
══════════════════════════════════════════════════════════ */
var _KAAVIO_TYOKALUT = [
  ...
  // Luokka C — puolustuspedagogiikan palat: mitä prässääjä ottaa pois (varjo), millä korkeudella
  // linja on, mikä tila on kohde.
  { k: 'peittovarjo',  lbl: 'Peittovarjo' },
  { k: 'korkeuslinja', lbl: 'Korkeuslinja' },
  { k: 'vyohyke',      lbl: 'Vyöhyke' },
  { k: 'selite',   lbl: 'Selite' },
  { k: 'poista',   lbl: 'Poista' }
];
...
function _kaavioTyokaluOhje() {
  ...
  if (t === 'peittovarjo') return _kuiT('Klikkaa peittäjää ja sitten peitettävää. Päät voi vaihtaa alta.');
  if (t === 'korkeuslinja') return _kuiT('Klikkaa korkeutta johon linja tulee. Raahaa linjaa pystysuunnassa.');
  if (t === 'vyohyke') return _kuiT('Klikkaa: vyöhyke päälle tai pois. Raahaa runkoa = siirrä, kulmaa = koko.');
...
function _kaavioValinnanNimi() {
  ...
  if (v.kind === 'varjo') return v.id + ' · ' + _kuiT('Peittovarjo');
  if (v.kind === 'linja') return v.id + ' · ' + _kuiT('Korkeuslinja');
  if (v.kind === 'vyohyke') return _kuiT('Vyöhyke');
  if (v.kind === 'liike') {
...
/* Osumatestin tyyppi → valintamallin kind. Kaksi nimeämistä on historiaa (osuma puhuu specin
   kenttänimillä, valinta lyhyemmillä) — tämä kartta pitää ne synkassa yhdessä paikassa. */
var _KAAVIO_OSUMA_KIND = { pelaaja: 'player', liike: 'liike', selite: 'selite',
...
function _kaavioOminaisuudetHTML() {
  ...
  } else if (v.kind === 'varjo') {
    // PÄIDEN VAIHTO VALITSIMISTA, ei uudelleenpiirrolla: varjon suunta (kuka peittää kenet) on
    // koko elementin merkitys, ja väärin päin oleva varjo opettaisi väärää asiaa.
    var pv = (m.spec.peittovarjot || []).filter(function (q) { return q.id === v.id; })[0];
    if (!pv) return h;
    h += _kaavioVarjoValitsinHTML('from', pv.from, _kuiT('Peittäjä'));
    h += _kaavioVarjoValitsinHTML('to', pv.to, _kuiT('Peitettävä'));
  } else if (v.kind === 'linja') {
    var kl = (m.spec.korkeuslinjat || []).filter(function (q) { return q.id === v.id; })[0];
    if (!kl) return h;
    // Numeeriset ± raahauksen rinnalle: mobiililla tarkka pystyraahaus on työlästä.
    h += rivi('<span style="font-size:11px;color:var(--ink3);align-self:center;min-width:54px">' + _kuiT('Korkeus') + ' ' + Math.round(kl.y) + '</span>'
      + _kvNappi('_kaavioOmLinja(-5)', '−') + _kvNappi('_kaavioOmLinja(5)', '+'));
  } else if (v.kind === 'vyohyke') {
    var z = m.spec.vyohyke;
    if (!z) return h;
    h += rivi('<span style="font-size:11px;color:var(--ink3);align-self:center;min-width:54px">' + _kuiT('Sijainti') + '</span>'
      + _kvNappi("_kaavioOmVyohyke('x',-5)", '←') + _kvNappi("_kaavioOmVyohyke('x',5)", '→')
      + _kvNappi("_kaavioOmVyohyke('y',-5)", '↑') + _kvNappi("_kaavioOmVyohyke('y',5)", '↓'));
    h += rivi('<span style="font-size:11px;color:var(--ink3);align-self:center;min-width:54px">' + _kuiT('Koko') + ' ' + Math.round(z.w) + '×' + Math.round(z.h) + '</span>'
      + _kvNappi("_kaavioOmVyohyke('w',-5)", '−') + _kvNappi("_kaavioOmVyohyke('w',5)", '+')
      + _kvNappi("_kaavioOmVyohyke('h',-5)", '↕−') + _kvNappi("_kaavioOmVyohyke('h',5)", '↕+'));
  }
  h += rivi(_kvNappi('_kaavioOmPoista()', 'Poista valittu', true));
  return h;
}

/* Peittovarjon pään valitsin. Sama pelaaja molemmissa päissä torjutaan editori-libissä; tässä
   se suodatetaan jo pois listalta, jottei käyttäjä edes näe virheellistä vaihtoehtoa. */
function _kaavioVarjoValitsinHTML(paa, valittu, otsikko) {
  var m = _kaavioTila.muokkaus, v = _kaavioTila.valittu, esc = _kuiEsc;
  var pv = ((m.spec.peittovarjot || []).filter(function (q) { return q.id === v.id; })[0]) || {};
  var toinen = (paa === 'from') ? pv.to : pv.from;
  var h = '<div style="margin-bottom:6px"><label style="display:block;font-size:10px;letter-spacing:.1em;text-transform:uppercase;color:var(--ink3);margin-bottom:3px">' + esc(otsikko) + '</label>';
  h += '<select onchange="_kaavioOmVarjoPaa(\'' + paa + '\', this.value)" style="' + _KV_KOHDE_TYYLI + ';padding:5px 8px;font-size:11.5px">';
  (m.spec.pelaajat || []).filter(function (p) { return p.id !== toinen; }).forEach(function (p) {
    h += '<option value="' + esc(p.id) + '"' + (p.id === valittu ? ' selected' : '') + '>'
      + esc(p.id + ' · ' + _kuiT(p.joukkue === 'vastustaja' ? 'vastustaja' : 'oma pelaaja')) + '</option>';
  });
  return h + '</select></div>';
}
function _kaavioOmVarjoPaa(paa, pelaajaId) {
  var m = _kaavioTila.muokkaus, v = _kaavioTila.valittu; if (!m || !v) return;
  _kaavioTila.historia = kaavioHistoriaLisaa(_kaavioTila.historia, m.spec);
  var r = kaavioAsetaPeittovarjoPaa(m.spec, v.id, paa, pelaajaId);
  if (r.virhe) { _kuiToast(r.virhe, 'err'); return; }
  m.spec = r.spec; _kaavioTyokaluPaivita(); _kaavioPiirraEditori();
}
function _kaavioOmLinja(delta) {
  var m = _kaavioTila.muokkaus, v = _kaavioTila.valittu; if (!m || !v) return;
  var kl = (m.spec.korkeuslinjat || []).filter(function (q) { return q.id === v.id; })[0]; if (!kl) return;
  _kaavioTila.historia = kaavioHistoriaLisaa(_kaavioTila.historia, m.spec);
  m.spec = kaavioSiirraKorkeuslinja(m.spec, v.id, kl.y + delta).spec;
  _kaavioTyokaluPaivita(); _kaavioPiirraEditori();
}
function _kaavioOmVyohyke(kentta, delta) {
  var m = _kaavioTila.muokkaus; if (!m || !m.spec.vyohyke) return;
  _kaavioTila.historia = kaavioHistoriaLisaa(_kaavioTila.historia, m.spec);
  var muutos = {}; muutos[kentta] = m.spec.vyohyke[kentta] + delta;
  m.spec = kaavioAsetaVyohyke(m.spec, muutos).spec;
  _kaavioTyokaluPaivita(); _kaavioPiirraEditori();
}
window._kaavioOmVarjoPaa = _kaavioOmVarjoPaa;
window._kaavioOmLinja = _kaavioOmLinja;
window._kaavioOmVyohyke = _kaavioOmVyohyke;
...
function _kaavioValitseTyokalu(k) {
  // Kesken jäänyt peittovarjon ensimmäinen pää ei saa roikkua työkalun vaihdon yli: seuraava
  // klikkaus toisella työkalulla luulisi jatkavansa edellistä elettä.
  _kaavioTila.varjoAlku = null;
...
function _kaavioPointerDown(e) {
  ...
  if (t === 'peittovarjo') {                            // C: klikkaa peittäjä → klikkaa peitettävä
    var ov = kaavioSnapPaate(s, sx, sy, 5);
    if (!ov.ref) { _kuiToast(_kuiT('Klikkaa pelaajaa'), 'err'); return; }
    // Kaksivaiheinen: ensimmäinen klikkaus jää odottamaan, toinen luo varjon. Odottava pää
    // näkyy valintana, jottei käyttäjä jää arvailemaan kumpi klikkaus oli ensimmäinen.
    if (!_kaavioTila.varjoAlku) {
      _kaavioTila.varjoAlku = ov.ref;
      _kaavioTila.valittu = { kind: 'player', id: ov.ref };
      _kaavioTyokaluPaivita(); _kaavioPiirraEditori();
      return;
    }
    var alku = _kaavioTila.varjoAlku; _kaavioTila.varjoAlku = null;
    if (alku === ov.ref) { _kuiToast(_kuiT('Peittovarjo: päät eivät voi olla sama pelaaja'), 'err'); _kaavioTyokaluPaivita(); return; }
    _kaavioTila.historia = kaavioHistoriaLisaa(_kaavioTila.historia, s);
    var rv = kaavioLisaaPeittovarjo(s, alku, ov.ref);
    if (rv.virhe) { _kuiToast(rv.virhe, 'err'); return; }
    m.spec = rv.spec;
    _kaavioAsetaValinta({ kind: 'varjo', id: rv.id });
    return;
  }
  if (t === 'korkeuslinja') {
    _kaavioTila.historia = kaavioHistoriaLisaa(_kaavioTila.historia, s);
    var rk = kaavioLisaaKorkeuslinja(s, sy);
    m.spec = rk.spec;
    _kaavioAsetaValinta({ kind: 'linja', id: rk.id });
    return;
  }
  if (t === 'vyohyke') {                                 // toggle: yksi vyöhyke per kaavio
    _kaavioTila.historia = kaavioHistoriaLisaa(_kaavioTila.historia, s);
    var paalla = !!s.vyohyke;
    m.spec = kaavioAsetaVyohyke(s, paalla ? false : { x: Math.max(0, sx - 20), y: Math.max(0, sy - 15), w: 40, h: 30 }).spec;
    _kaavioAsetaValinta(paalla ? null : { kind: 'vyohyke', id: 'vyohyke' });
    return;
  }
  if (t === 'poista') {
  ...
  if (osuma.ref) { _kaavioTila.drag = { kind: 'player', id: osuma.ref }; e.currentTarget.setPointerCapture(e.pointerId); return; }
  // LUOKKA C on raahattavissa samalla eleellä: korkeuslinja pystysuunnassa, vyöhyke runko/kulma.
  // Tarttuma talteen, jotta vyöhyke ei hyppää kursorin alle vaan liikkuu suhteessa otteeseen.
  var oc = (typeof kaavioOsuma === 'function') ? kaavioOsuma(s, sx, sy, 5) : null;
  if (oc && (oc.tyyppi === 'korkeuslinja' || oc.tyyppi === 'vyohyke')) {
    _kaavioTila.drag = (oc.tyyppi === 'korkeuslinja')
      ? { kind: 'linja', id: oc.id }
      : { kind: 'vyohyke', id: 'vyohyke', kulma: !!oc.kulma, dx: sx - s.vyohyke.x, dy: sy - s.vyohyke.y };
    e.currentTarget.setPointerCapture(e.pointerId);
  }
}
function _kaavioPointerMove(e) {
  ...
  var d = _kaavioTila.drag, sp = _kaavioTila.muokkaus.spec;
  var sx2 = kaavioEdOX(xy[0]), sy2 = kaavioEdOY(xy[1]);
  var r;
  if (d.kind === 'linja') r = kaavioSiirraKorkeuslinja(sp, d.id, sy2);
  else if (d.kind === 'vyohyke') {
    r = d.kulma
      ? kaavioAsetaVyohyke(sp, { w: sx2 - sp.vyohyke.x, h: sy2 - sp.vyohyke.y })
      : kaavioAsetaVyohyke(sp, { x: sx2 - d.dx, y: sy2 - d.dy });
  } else r = kaavioSiirraPelaaja(sp, d.id, xy[0], xy[1]);
  if (!r.virhe) { _kaavioTila.muokkaus.spec = r.spec; _kaavioPiirraEditori(); }
}
function _kaavioPointerUp(e) {
  ...
  if (_kaavioTila.drag && _kaavioTila.klikAlku && _kaavioTila.klikAlku.liikkui) {
    ...
    _kaavioTila.valittu = osui ? { kind: _KAAVIO_OSUMA_KIND[osui.tyyppi] || osui.tyyppi, id: osui.id } : null;
    _kaavioTyokaluPaivita();
  }
...
/* JÄLKIRIKASTUS — kaavio pelihavainnon todisteeksi.
   Kun editori on avattu havainnosta, `m._liitaHavaintoon` kantaa kontekstin
   { seuraId, pelaajaId, havaintoId }. Linkki kirjoitetaan HAVAINNON puolelle
   (lib/tm_havainto_kaavio.js) heti kun kaaviolla on id — sama päivityskuvio
   kuin ADARin AI-narratiivilla, ei sääntömuutosta, ei `luotu`-kosketusta (A5).
   HUOM: linkin kirjoitus tapahtuu tuossa libissä, EI täällä — tämän tiedoston
   jokainen kaaviokirjoitus nostaa version (kaavio_moduulit-portti valvoo sitä
   merkkijonotasolla, joten täällä ei mainita päivitysmetodia kirjaimellisesti).
   Kirjoitetaan KERRAN: `_liitetty`-lippu estää toiston kun editori jää auki ja
   käyttäjä painaa Tallenna uudelleen. Best-effort — kaavio on jo tallessa. */
async function _kaavioLiitaHavaintoon(m) {
  try {
    var ctx = m && m._liitaHavaintoon;
    if (!ctx || m._liitetty || !m.id) return;
    if (typeof _kuiDb !== 'function' || typeof tmHavaintoKaavio === 'undefined') return;
    var ok = await tmHavaintoKaavio.liita(_kuiDb(), ctx, m.id);
    if (ok) {
      m._liitetty = true;
      _kuiToast(_kuiT('Kaavio liitetty havaintoon'), 'ok');
      /* Havaintonäkymä (Master/VP) päivittää itsensä tästä — ei tiukkaa kytkentää. */
      try { if (typeof document !== 'undefined') document.dispatchEvent(new CustomEvent('tm:havainto:kaavio', { detail: { ctx: ctx, kaavioId: m.id } })); } catch (e) { /* ei kriittinen */ }
    }
  } catch (e) { /* linkitys ei saa kaataa tallennusta */ }
}
...
async function _kaavioTallenna() {
  ...
  if (m._uusi || !m.id) {
    try {
      ...
      await _kaavioLiitaHavaintoon(m);  // jälkirikastus: linkki havaintoon jos konteksti on
  ...
  try {
    ...
    await _kaavioLiitaHavaintoon(m);   // myös olemassa oleva kaavio voidaan liittää havaintoon
    _kaavioTallennettuLeima();
